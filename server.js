...
// ─── INLINE DATA ──────────────────────────────────────────────────────────────
// Postcode → GRD mapping (volledig België)
const POSTCODE_GRD = {};
...
// Tarieven per GRD (LS-standaard, gecalibreerd op Fluvius West referentie)
const TARIEVEN = {
  'Fluvius West': {
...
// Andere GRDs krijgen Fluvius West als default (conservative fallback)
const GRD_DEFAULT = TARIEVEN['Fluvius West'];

// Contract staffel — Enwyse (gecalibreerd)
...
// Batterijen — standaard Fluctus portfolio
let BATTERIJEN = [
...
// In-memory opslag voor scenarios en projecten (reset na herstart)
const SCENARIOS_DB = {};
const PROJECTEN_DB = new Set();

// ─── HEALTH ──────────────────────────────────────────────────────────────────
app.get('/', (req, res) => res.json({ status: 'ok', version: '15.6', ts: new Date().toISOString() }));
...
app.get('/api/postcode-grd', (req, res) => {
  ...
  const hit = POSTCODE_GRD[pc];
  if (!hit) {
    // Fallback: probeer 10-eenheid
    const hit2 = POSTCODE_GRD[String(Math.floor(parseInt(pc) / 10) * 10)];
    if (hit2) return res.json({ postcode: pc, grd: hit2.grd, dnb_volledig: hit2.dnb, gemeenten: [], fallback_naar: hit2.grd });
    return res.status(404).json({ error: `Postcode ${pc} niet gevonden` });
  }
...
app.get('/api/gemeenten-lijst', (req, res) => {
  // Minimale lijst — snippet gebruikt dit voor autocomplete
  // In productie zou dit een volledige 2500-gemeenten lijst zijn
...
app.get('/api/regio-tarieven', (req, res) => {
  ...
  const t = TARIEVEN[grd] || GRD_DEFAULT;
  res.json({
    grd, spanning,
    tarieven: {
      ...
      federale_heffing: (t.accijnzen_staffel && t.accijnzen_staffel[0]) ? t.accijnzen_staffel[0][1] : 15.08,
    },
    raw: t
  });
});

// ─── LEVERINGSCONTRACT STAFFEL ───────────────────────────────────────────────
app.get('/api/leveringscontract-staffel', (req, res) => {
  res.json({
    leverancier: 'Enwyse',
    schijven: CONTRACT_STAFFEL,
    staffel: CONTRACT_STAFFEL,
    vergroening_eur_per_mwh: 2.50,
    vast_eur_per_maand: 10.00,
    gsc_eur_mwh: 11.0,
    wkk_eur_mwh: 4.20,
  });
});
...
app.get('/api/profielen-lijst', (req, res) => {
  res.json({ profielen: PROFIELEN });
});

app.get('/api/profiel', (req, res) => {
  const naam = req.query.naam || '';
  const profielPath = path.join(__dirname, 'data', naam + '.json');
  // Probeer eerst data/<naam>.json
  if (fs.existsSync(profielPath)) {
    const data = JSON.parse(fs.readFileSync(profielPath, 'utf8'));
    return res.json({ naam, kwartier: data.profiel_kwartier || data });
  }
  // Fallback: slager.json in root
  const slagerPath = path.join(__dirname, 'slager.json');
  if (fs.existsSync(slagerPath)) {
    const data = JSON.parse(fs.readFileSync(slagerPath, 'utf8'));
    return res.json({ naam, kwartier: data });
  }
...
app.get('/api/batterijen', (req, res) => {
  res.json({ batterijen: BATTERIJEN });
});
...
app.get('/api/scenarios', (req, res) => {
  ...
  const scenarios = Object.keys(SCENARIOS_DB[project] || {});
  res.json({ scenarios });
});

app.get('/api/scenario', (req, res) => {
  const { project, scenario } = req.query;
  const data = (SCENARIOS_DB[project] || {})[scenario];
  if (!data) return res.status(404).json({ error: 'Scenario niet gevonden' });
  res.json({ data });
});
...
app.post('/api/nominatie-sim', (req, res) => {
  ...
  // Bouw volledige simulator-input uit UI-input
  // UI stuurt een vereenvoudigd formaat — server vertaalt naar volledig simulator-formaat
  ...
  proc.on('close', code => {
    ...
    let result;
    try { result = JSON.parse(stdout.slice(jsonStart, jsonEnd + 1)); }
    catch (e) { return res.status(500).json({ error: 'JSON parse fout', detail: e.message }); }
...
// ─── BUILD SIM INPUT ─────────────────────────────────────────────────────────
// Vertaalt UI-input (vereenvoudigd) naar volledig simulator.py input formaat
function buildSimInput(ui) {
  const grd = ui.grd || 'Fluvius West';
  const spanning = ui.spanning || 'LS';
  const tarieven = TARIEVEN[grd] || GRD_DEFAULT;

  // Contract staffel ophalen of UI-versie gebruiken
  const staffel = (ui.contract && ui.contract.staffel && ui.contract.staffel.length > 0)
    ? ui.contract.staffel
    : CONTRACT_STAFFEL;

  // Batterij ophalen uit UI of cache
  ...
  if (ui.batterijId) {
    ...
    if (b) batt = { kw: b.kw, kwh: b.kwh, dod_pct: Math.round((b.dod||0.90)*100), rte_pct: Math.round((b.eta||0.85)*100), capex_eur: b.capex||0, max_cycli: b.max_cycli||8000 };
  }

  // Profiel: UI stuurt profielNaam, server haalt het op als het al via /api/profiel werd gecached
  // Simulator verwacht profiel_kwartier[35040]. Als UI het niet stuurt: gebruik lege array (simulator fallback)
  const profielKwartier = ui.profiel_kwartier || [];

  const pvKwp = ui.pv_kwp || ui.pvKwp || 0;
  const aanslKva = ui.aansluiting_kva || ui.aansluitingKva || 80;
  const aanslKw  = aanslKva; // kVA ≈ kW voor PF=1

  // BSP / stacked bepalen
  const bspActief  = !!(ui.bsp && ui.bsp.actief);
  const stacked    = batt.kwh > 0; // met batterij altijd stacked dispatch
  ...
  return {
    ...
    jaarverbruik_mwh: ui.jaarverbruik_mwh || ui.jaarverbruik || 200,
    aanvullingen: {},
    pv: {
      kwp: pvKwp,
      specifiek_rendement_kwh_per_kwp: 900,
      vorm_kwartier: ui.pv_vorm_kwartier || [],
    ...
    batterij: batt,
    aansluiting: {
      max_afname_kw_zacht: aanslKw,
      max_afname_kw_hard:  aanslKw,
      max_injectie_kw_zacht: aanslKw,
      max_injectie_kw_hard:  aanslKw,
      tarief_overschrijding_afname_eur_per_kw_jaar: tarieven.overschrijding_toegangsvermogen_eur_kw_jaar || 62.47,
    ...
    forecast: {
      sigma_da: 0, sigma_imb: 0,
      sigma_volume_verbruik_pct: 0, sigma_volume_pv_pct: 0,
    },
    markt: {
      spot_kwartier: ui.spot_kwartier || [],
      imb_kwartier:  ui.imb_kwartier  || [],
    },
...
app.listen(PORT, () => {
  ...
  console.log(`Postcodes geladen: ${Object.keys(POSTCODE_GRD).length}`);
});
