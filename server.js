...
app.get('/api/kbo', async (req, res) => {
  try {
    ...
    if (String(req.query.debug || '') === '1') {
      ...
      try { const r = await fetch(url, { headers }); status = r.status; body = (await r.text()).slice(0, 700); } catch (e) { body = 'FETCH-ERROR: ' + e.message; }
