...
async function _graphInboundSweep() {
  ...
  const apiKey = process.env.ANTHROPIC_API_KEY;
  let mails;
  try { mails = await graphInbound.fetchUnread(); }   // token + ophalen; faalt bij verlopen geheim / weg consent / foute ENV
  catch (e) { await _graphAuthAlert(e); return { ok: false, error: e.message, fase: 'inloggen/ophalen' }; }
  const uit = [];
  console.log(`[graph-inbound] sweep: ${mails.length} ongelezen mail(s) met bijlage`);
  for (const m of mails) {
    if (_graphGezien.has(m.id)) { console.log(`[graph-inbound] mail van ${m.from} al gezien deze runtime — overslaan`); continue; }
    _graphGezien.add(m.id);
    const res = { id: m.id, from: m.from, subject: m.subject, status: null };
    try {
      ...
      if (!pdfs.length) { res.status = 'geen_pdf'; await graphInbound.markRead(m.id); uit.push(res); console.log(`[graph-inbound] → geen PDF, gemarkeerd als gelezen`); continue; }
      if (!apiKey) { res.status = 'geen_apikey'; uit.push(res); console.error(`[graph-inbound] → GEEN ANTHROPIC_API_KEY — kan factuur niet lezen (niet gemarkeerd, retry volgt)`); continue; }   // niet markeren → later opnieuw
      ...
      console.log(`[graph-inbound] → status=${res.status}${res.token ? (', token=' + res.token) : ''} — mail gemarkeerd als gelezen`);
    } catch (e) {
      res.status = 'fout'; res.error = e.message;   // niet markeren → volgende sweep herprobeert
      console.error(`[graph-inbound] mail-FOUT (${m.from} · "${m.subject}"): ${e.message} — NIET gemarkeerd, retry bij volgende sweep`);
    }
    uit.push(res);
  }
...
(function _startGraphInboundScheduler() {
  ...
  if (graphInbound.graphEnabled()) console.log(`[graph-inbound] scheduler actief — poll elke ${min} min op ${process.env.GRAPH_MAILBOX}`);
})();
