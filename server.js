...
// sendMail, energiekompas@fluctus.net): een échte reply uit de mailbox, zonder mailinglijst-banner. Valt terug op
...
async function _ekKlantMail(to, subject, textContent, htmlContent) {
  ...
  const r = await _brevoMailRaw(to, subject, textContent, htmlContent, 'Fluctus EnergieKompas');
...
app.post('/api/ek-rapport-mail', async (req, res) => {
  try {
    ...
    const kHtml = `<p>Beste${esc2(naam)},</p><p>Uw EnergieKompas-rapport staat klaar:<br><a href="${esc2(link)}">${esc2(link)}</a></p><p>Zodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.</p><p>Met vriendelijke groeten,<br>EnergieKompas</p>`;
    let sent = false; try { const r = await _ekKlantMail(mail, 'Uw EnergieKompas-rapport', kTxt, kHtml); sent = !!(r && r.sent); } catch (e) {}
