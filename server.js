...
app.post('/api/lead-afsluiten', async (req, res) => {
  try {
    const b = req.body || {}; const rec = _leadLezen(b.token);
    if (!rec) return res.status(404).json({ ok: false, error: 'Lead niet gevonden of verlopen.' });
    const partner = !!b.partner_offerte && !!b.voorschot_betaald, ems = !!b.ems;
    ...
    rec.afsluiting = { partner_offerte: !!b.partner_offerte, voorschot_betaald: !!b.voorschot_betaald, ems: ems, segment: seg, kickback_bedrag: bedrag, ts: Date.now() };
    if (partner) { rec.kickback = { partner: rec.partner || '', segment: seg, bedrag: bedrag, voorschot_betaald: true, status: 'voorschot_bevestigd', ts: Date.now() }; _leadEvent(rec, 'kickback_getriggerd', {}, b.token); }
    ...
    const acties = [];
    if (partner) acties.push(`→ FACTUREER kickback ${_eurTxt(bedrag)} (segment ${seg || '?'}) en START de KU Leuven-verificatie op de partnerofferte (bankdossier + investeringsaftrek).`);
    if (ems) acties.push('→ EMS-order Voltmasters: eenmalig € 2.000 + € 6/kVA/jaar; plan de evaluatie op 6 en 12 maand (telemetrie).');
