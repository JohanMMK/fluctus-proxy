...
async function _notifyKlantAangevraagd(en, klantMail, factuuradres) {
  try {
    ...
    const txt = `Beste,\n\nWij hebben voor uw EAN (${en.ean}) toegang tot uw meetgegevens aangevraagd bij Fluvius.\n\n` +
      ...
      `Vindt u die mail niet terug? Dan stuurt Fluvius binnen enkele dagen een brief met een QR-code${adr}, waarmee u de bevestiging ook kan geven.\n\n` +
    ...
    const r = await _brevoMail(klantMail, 'Bevestig de toegang tot uw meetgegevens (Fluvius)', txt, null, null, 'mandaat-aangevraagd');
...
const _MAIL_TEMPLATES = {
  'nota': 'Onderhandelingsnota (na factuur-upload)',
  'mandaat-organiseren': 'Wij organiseren uw mandaat',
  'studie-klaar': 'Studie/herberekening staat klaar',
...
app.post('/api/lead-herbereken', async (req, res) => {
  try {
    ...
    const kSent = await _brevoMail(rec.mail, kop, kTxt, null, null, 'studie-klaar');
