...
app.post('/api/lead-afsluiten', async (req, res) => {
  try {
    ...
    if (partner) {
      ...
      rec.vijfde_analyse = { basis: 'winnende partnerofferte', kul_verificatie: true, status: 'te_starten', ts: Date.now() };
      _leadEvent(rec, 'kickback_getriggerd', {}, b.token); _leadEvent(rec, 'vijfde_analyse_gevraagd', {}, b.token);
    } else {
    ...
    const acties = [];
    if (partner) acties.push(`→ START de 5de analyse op de winnende PARTNER-offerte incl. KU Leuven-verificatie (bankdossier + investeringsaftrek) en factureer de kickback ${_eurTxt(bedrag)} (segment ${seg || '?'}).`);
