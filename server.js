...
app.get('/api/kamino/projecten-overzicht', async (req, res) => {
  ...
  try {
    ...
    for (const rec of recs) {
      ...
      const mandaatEans = (rec.mandaat && Array.isArray(rec.mandaat.eans)) ? rec.mandaat.eans : [];
      ...
      if (hit) {
        ...
        if (rec.abonnement_opt_out) {
          ...
          const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
            ...
            <p>Met vriendelijke groeten,<br>EnergieKompas</p></div>`;
          const kSent = await _brevoMail(rec.mail, 'Uw energie-analyse is bijgewerkt', kTxt, kHtml, null, 'abonnement-bijgewerkt');
        ...
        if (autoreport) {
          ...
          const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
            ...
            <p>Met vriendelijke groeten,<br>EnergieKompas</p></div>`;
          const kSent = await _brevoMail(m.from, 'Uw energie-analyse staat klaar', kTxt, kHtml, null, 'abonnement-nieuw');
