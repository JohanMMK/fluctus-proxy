...
// Zoekt/bouwt de conversie-spec: builtin → registry → AI (max 2 pogingen) → heuristiek. Test elke kandidaat op het VOLLEDIGE bestand.
async function _piSpecVoor(bestand, rol) {
...
async function _piAnalyseKern(u, b, ctx) {
  ...
  for (const d of decoded) {
    const r = await _piSpecVoor(d.best, d.rol);
    if (r.fout) {
...
async function _ekKlantMail(to, subject, textContent, htmlContent) {
  ...
  if (kanaal === 'graph' && graphInbound.graphEnabled()) {
    try { await graphInbound.sendMail(to, subject, htmlContent || null, textContent || ''); return { sent: true, kanaal: 'graph' }; }
    catch (e) { console.warn('[ek-klantmail] Graph faalde, val terug op Brevo:', e.message); }
    const r = await _brevoMailRaw(to, subject, textContent, htmlContent, 'Energie-Compas');
  ...
  if (graphInbound.graphEnabled()) {
    try { await graphInbound.sendMail(to, subject, htmlContent || null, textContent || ''); return { sent: true, kanaal: 'graph-fallback' }; }
...
app.post('/api/ek-rapport-mail', async (req, res) => {
  try {
    ...
    const kHtml = `<p>Beste${esc2(naam)},</p><p>Uw Energie-Compas-rapport staat klaar:<br><a href="${esc2(link)}">${esc2(link)}</a></p><p>Zodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.</p><p>Met vriendelijke groeten,<br>Energie-Compas</p>`;
    let sent = false; try { const r = await _ekKlantMail(mail, 'Uw Energie-Compas-rapport', kTxt, kHtml); sent = !!(r && r.sent); } catch (e) {}
    if (rec) { rec.ek_rapport_mail_ts = nu; _leadOpslaan(token, rec); try { _leadEvent(rec, 'ek_rapport_mail', {}, token); } catch (e) {} }
    return res.json({ ok: true, mail, sent, throttled: false });
