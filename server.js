...
//                second opinion op tegen de Energie-Compas-studie van de lead: investering/jaarkost/PV/batterij/laadpalen,
...
//                scan.financieel draagt ze mee → voedt het factuur-paneel + de Energie-CompasImpact (EKI = energiekost/
...
//                draaien; project_id komt in het lead-antwoord én de lead-notificatiemail. ── v15.111.0 (2026-08-30, Fase 4 — Energie-Compas €/km ALL-IN): /api/energiekompas/kpi rekent de
...
//                net_heffing_eur_mwh toegevoegd. ── v15.110.0 (2026-08-30, Fase 4 — Energie-Compas LEAD-CAPTURE): nieuwe publieke endpoints /api/lead
...
//                ENV: LEAD_MAIL_TO (default = AUDIT_MAIL_TO), WEB_BASE (default https://app.energie-compas.eu). Geen wijziging
...
//                de Energie-Compas-nota, gevoed met de factuur-baseCase, identieke cijfers als Kamino tegel 1.
...
// Versie:        v15.102.0 (2026-08-27, Fase 4 — Energie-Compas publieke KPI's): nieuw PUBLIEK (geen login)
...
//                enkel de **Energie-Compas**-schil (app_id 'energiekompas'), niet de losse interne tools.
...
//                v15.95.0 (enkel Energie-Compas) — zie boven.
...
// ─── CONTRACT-HEATMAPS (Kamino-tegel 1 / Energie-Compas onderhandelingsnota) ──────
...
const BREVO_SENDER  = process.env.BREVO_SENDER  || 'dossier@energie-compas.eu';
...
// Energie-Compas-schil — niet de losse interne tools. Energie-Compas is de partner-
...
async function _heeftAppToegang(u, appId) {
  ...
  // v15.95 (Fase 2b): partnermanager = automatische toegang tot ENKEL Energie-Compas.
...
// ─── v15.102 (Fase 4 — Energie-Compas publieke KPI's) ─────────────────────────────────────────
// PUBLIEK (geen login): eerste-feedback-KPI's voor de Energie-Compas-schil, op de LIVE markt (MARKT.spot_q,
...
// ─── Energie-Compas bedrijf — gedeelde context (coarse sweep + losse cel) ──────
...
function _ekBedrijfCtx(b) {
  ...
  const baseUiGebouw = () => _mkBaseUi([]);          // v15.166: gebouw-only (GEEN laadpleinen) — de "vandaag"-referentie (laden gebeurt elders @ CREG)
  // v15.166 (Johan): laadvolume wagenpark (MWh/jaar) — de energie die vandaag ELDERS aan CREG-tarief geladen wordt.
  //   Enkel wagenpark-pleinen (geen betalende bezoekers). Zelfde formule als de sweep-income (aantal × km/jaar × kWh/km).
  let evMwh = 0;
  pleinen.forEach(p => { if (String(p.type_plein) === 'bezoekers') return; const aantal = Math.max(1, _n(p.aantal) || 1); const kmPer = _n(p.km_per_jaar); const kwhkm = _n(p.kwh_per_km) || 0.16; evMwh += aantal * kmPer * kwhkm / 1000; });
  const CREG_MWH = 322;   // regulier laadtarief (CREG) — de referentie voor "laden elders"
  return { afname, kva, spanning, toegang, profielNaam, grd, postcode, pleinen, pleinenSim, genKva, generator, lpKw, bessMaxKw, pvMaxKwp, PV_KOST, BATT_KWH_KOST, CRATE, baseUi, baseUiNoGen, baseUiGebouw, evMwh, CREG_MWH };
}
...
async function _ekBedrijfCel(ctx, pv, bkw) {
  ...
  // v15.163: volledige batterij-cycli/jaar = jaarlijkse ontlaadenergie / bruikbare capaciteit (voor de heatmap-parameter).
  const capKwh = bkw > 0 ? bkw * ctx.CRATE : 0;
  const ontl = (r.profielen && r.profielen.batt_ontladen_kw) || [];
  const cycli = capKwh > 0 ? Math.round(ontl.reduce((s, v) => s + (+v || 0), 0) * 0.25 / capKwh) : 0;
  return { battKw: bkw, battKwh: bkw * ctx.CRATE, pvKwp: pv, kost: Math.round(kost), capex: Math.round(capex), cycli };
}
...
app.post('/api/energiekompas/bedrijf-sweep', async (req, res) => {
  try {
    ...
    if (ctx.generator) {
      ...
      // v15.166 (Johan): referentie "vandaag" = gebouwverbruik ZONDER laden op-site + de wagens laden ELDERS @ CREG (322 €/MWh).
      //   De sweep-cellen draaien mét laden áán het gebouw, geoptimaliseerd met PV+batt+sturing. Zo telt besparing = ref − cel
      //   zowel de verschoven laadkost (CREG → goedkoop on-site) als de gebouw-optimalisatie mee, ZONDER de kunstmatige piek
      //   van dom on-site laden in de referentie. (Vroeger was de referentie de (0,0)-cel = gebouw + laden on-site dom.)
      const uiGeb = ctx.baseUiGebouw(); uiGeb.pv_kwp = 0; uiGeb.batterijId = ''; uiGeb.batterijCustom = null;
      const rGeb = await _runSimulatorOnce(buildSimInput(_variantUi(uiGeb, 'geen')));
      const gebKost = Number((rGeb.jaarfactuur || rGeb.factuur || {}).subtotaal_excl_btw) || 0;
      refKost = Math.round(gebKost + ctx.evMwh * ctx.CREG_MWH);
    }
    ...
    // v15.157.2: MOBILITEITSBESPARING uit de engine — CREG-referentie (322 €/MWh) + dieselreferentie (9 €/100km) op
    //   het laadplein-volume (aantal × km/jaar × kWh/km). Zelfde referenties als thuisladen; beide meegegeven zodat de
    //   EK-schil de besparing vs diesel én vs regulier laden toont. Het laden zelf blijft in de sim binnen het
    //   toegangsvermogen (dispatch onder de aansluiting). Alleen 'wagenpark'-pleinen tellen mee (geen betalende bezoekers).
    const _plGuard = Array.isArray((req.body || {}).pleinen) ? req.body.pleinen : [];
    const CREG_MWH = 322, DIESEL_100 = 9;
    let evMwh = 0, kmTotAll = 0;
    _plGuard.forEach(p => {
      if (String(p.type_plein) === 'bezoekers') return;   // betalend plein = geen eigen wagenpark-besparing
      const aantal = Math.max(1, Number(p.aantal) || 1);
      const kmPer = Number(p.km_per_jaar) || 0, kwhkm = Number(p.kwh_per_km) || 0.16;
      const kmTot = aantal * kmPer; kmTotAll += kmTot; evMwh += kmTot * kwhkm / 1000;
    });
    const income = { referentiekost: refKost, creg: Math.round(evMwh * CREG_MWH), diesel: Math.round((kmTotAll / 100) * DIESEL_100), ev_mwh: Math.round(evMwh * 10) / 10 };
    ...
    return res.json({ ok: true, anchors, income, grid: { batt_kw_as: bkwAs, pv_kwp_as: pvAs }, referentie_kost: refKost,
...
// ─── v15.163: KLANTRAPPORT-ENDPOINT ──────────────────────────────────────────
// Levert de weergaveklare data voor /apps/klantrapport.html voor ÉÉN gekozen combi.
// Draait 4 sims PARALLEL: nul (pv0/batt0, dom laden) + de gekozen combi in 3 sturingen
// (geen/sturing/onbalans). Aggregeert de per-kwartier-profielen (35040) naar uur-heatmaps
// [24][365] en bouwt de factuurvergelijking (CREG-nul vs. PV+batt) + batterij-cycli/jaar.
// Rekent NIETS in de schil; alle cijfers komen uit simulator.py. Anti-regressie: onbalans =
// passieve respons (imbalance settlement) + spot-arbitrage; geen reservediensten.
function _uurMat(arr) {                       // 35040 kwartieren → [24][365] uurgemiddelde (kW) / niveau
  const H = 24, D = 365, m = Array.from({ length: H }, () => new Array(D).fill(0));
  if (!Array.isArray(arr) || !arr.length) return m;
  const n = Math.min(arr.length, 35040);
  for (let i = 0; i < n; i++) { const day = Math.floor(i / 96); if (day >= D) break; const hr = Math.floor((i % 96) / 4); m[hr][day] += (+arr[i] || 0); }
  for (let h = 0; h < H; h++) for (let d = 0; d < D; d++) m[h][d] = Math.round(m[h][d] / 4 * 10) / 10;
  return m;
}
function _uurMatPos(arr) { return _uurMat((arr || []).map(v => Math.max(0, +v || 0))); }   // afname (netto > 0)
function _uurMatNeg(arr) { return _uurMat((arr || []).map(v => Math.max(0, -(+v || 0)))); } // injectie (netto < 0)
function _sumLaadpleinLoad(r) {              // Σ EV-load per kwartier over alle pleinen
  const pls = (r && r.laadplein && r.laadplein.pleinen) || [], N = 35040, out = new Array(N).fill(0);
  pls.forEach(p => { const lk = (p && p.load_kw) || []; const n = Math.min(N, lk.length); for (let i = 0; i < n; i++) out[i] += (+lk[i] || 0); });
  return out;
}
function _factuurBits(r) {                   // sim-jaarfactuur → klantrapport-posten (excl. btw)
  const jf = (r && r.jaarfactuur) || {}, g = jf.groepen || {}, sub = x => (x && x._subtotaal) || 0;
  return {
    energie:   Math.round(sub(g.A_energiekost)),                                                  // energie incl. laden, na injectie-energieverrekening
    netkosten: Math.round(sub(g.B_netgebruik_afname) + sub(g.C_netgebruik_injectie) + sub(g.D_transport)),
    heffingen: Math.round(sub(g.E_heffingen)),
    injectie_opbrengst: Math.round(jf.injectie_opbrengst_eur || 0),
    saldo:     Math.round(jf.subtotaal_excl_btw || 0)
  };
}
async function _ekRapportRun(ctx, pv, bkw, variant) {
  const ui = ctx.baseUi(); ui.pv_kwp = pv;
  if (bkw > 0) { ui.batterijId = 'CUSTOM'; ui.batterijCustom = { naam: 'ek-bedrijf', kw: bkw, kwh: bkw * ctx.CRATE, aantal_batterijen: 1, dod_pct: 90, rte_pct: 92, capex_eur: 0, max_cycli: 8000 }; }
  else { ui.batterijId = ''; ui.batterijCustom = null; }
  const simInput = buildSimInput(_variantUi(ui, variant));
  simInput.emit_batt_profielen = true;   // v15.165: enkel het klantrapport heeft de batterij-profielen nodig (SOC/ontladen/laden + cycli). Elders weggelaten → houdt de sweep-payload klein.
  return await _runSimulatorOnce(simInput);
}
// v15.166 (Johan): gebouw-only referentie-run (GEEN laadpleinen), pv0/batt0, dom → de "vandaag"-factuur van enkel het gebouw.
//   De wagens laden in die referentie ELDERS @ CREG; die laadkost tellen we apart bij de nul-factuur (zie _ekBedrijfRapport).
async function _ekRapportRunGebouw(ctx) {
  const ui = ctx.baseUiGebouw(); ui.pv_kwp = 0; ui.batterijId = ''; ui.batterijCustom = null;
  return await _runSimulatorOnce(buildSimInput(_variantUi(ui, 'geen')));
}
async function _ekBedrijfRapport(ctx, pv, bkw) {
  const [rNul, rGeb, rGeen, rStuur, rOnb] = await Promise.all([
    _ekRapportRun(ctx, 0, 0, 'geen'),        // heatmap-referentie: geen PV/batt, laden ON-SITE (dom) — voedt de afname/laden-heatmaps
    _ekRapportRunGebouw(ctx),                // v15.166 FACTUUR-referentie: gebouw-only (geen laden op-site) + laden elders @ CREG
    _ekRapportRun(ctx, pv, bkw, 'geen'),     // gekozen combi, ongestuurde injectie
    _ekRapportRun(ctx, pv, bkw, 'sturing'),  // gekozen combi, spot-sturing (batterij-profielen)
    _ekRapportRun(ctx, pv, bkw, 'onbalans')  // gekozen combi, incl. onbalans
  ]);
  const P = r => (r && r.profielen) || {}, verm = r => P(r).vermogen_aansluiting_kw || [];
  const nulLp = _sumLaadpleinLoad(rNul), nulVerm = verm(rNul);
  const asisArr = nulVerm.map((v, i) => Math.max(0, Math.max(0, +v || 0) - (nulLp[i] || 0)));   // gebouw as-is = afname − EV-load
  const capKwh = bkw > 0 ? bkw * ctx.CRATE : 0;
  const ontlSum = (P(rStuur).batt_ontladen_kw || []).reduce((s, v) => s + (+v || 0), 0) * 0.25;  // kWh/jaar ontladen
  const cycli = capKwh > 0 ? Math.round(ontlSum / capKwh) : null;
  const heat = {
    injectie_gewoon:     { mat: _uurMatNeg(verm(rGeen)),  eenheid: 'kW' },
    injectie_spot:       { mat: _uurMatNeg(verm(rStuur)), eenheid: 'kW' },
    injectie_onbalans:   { mat: _uurMatNeg(verm(rOnb)),   eenheid: 'kW' },
    laadplein: ((rStuur.laadplein && rStuur.laadplein.pleinen) || []).map(p => ({ naam: p.naam || 'Laadplein', mat: _uurMat(p.load_kw || []), eenheid: 'kW' })),
    afname_asis:         { mat: _uurMat(asisArr),         eenheid: 'kW' },
    afname_laden_zonder: { mat: _uurMatPos(verm(rNul)),   eenheid: 'kW' },
    afname_met_sturing:  { mat: _uurMatPos(verm(rStuur)), eenheid: 'kW' }
  };
  if (capKwh > 0) {
    heat.batt_soc      = { mat: _uurMat(P(rStuur).soc_kwh || []),          eenheid: 'kWh' };
    heat.batt_ontladen = { mat: _uurMat(P(rStuur).batt_ontladen_kw || []), eenheid: 'kW' };
    heat.batt_laden    = { mat: _uurMat(P(rStuur).batt_laden_kw || []),    eenheid: 'kW' };
  }
  // v15.166 (Johan): nul-factuur = gebouwverbruik (rGeb, ZONDER laden op-site) + de wagens laden ELDERS @ CREG.
  //   De laadkost @ CREG telt bij de energiepost. fOpt = gekozen combi met laden áán het gebouw, geoptimaliseerd (PV+batt+sturing).
  //   Zo is de besparing consistent met de EK-heatmap (dezelfde CREG-referentie) en niet gebaseerd op dom on-site laden.
  const fGeb = _factuurBits(rGeb);
  const cregLaden = Math.round(ctx.evMwh * ctx.CREG_MWH);
  const fNul = Object.assign({}, fGeb, { energie: fGeb.energie + cregLaden, saldo: fGeb.saldo + cregLaden, laden_creg: cregLaden });
  const fOpt = _factuurBits(rStuur);
  const besp = fNul.saldo - fOpt.saldo;
  const factuur = { nul: fNul, opt: fOpt, besparing_jaar: besp, besparing_pct: fNul.saldo > 0 ? Math.round(besp / fNul.saldo * 100) : 0, laden_creg: cregLaden, creg_eur_mwh: ctx.CREG_MWH };
  const capex = Math.round(pv * ctx.PV_KOST + capKwh * ctx.BATT_KWH_KOST);
  return {
    factuur, heat, cycli,
    combi: { pv_kwp: pv, batt_kwh: capKwh },
    financieel: { investering: capex, besparing_jaar: besp, batt_cycli_jaar: cycli },
    heeftLaadplein: !!(rStuur.laadplein && (rStuur.laadplein.pleinen || []).length)
  };
}
app.post('/api/energiekompas/bedrijf-rapport', async (req, res) => {
  try {
    if (!MARKT) return res.status(503).json({ error: 'Marktdata nog niet geladen — probeer over 30 s opnieuw', markt_status: MARKT_STATUS });
    const b = req.body || {}; const ctx = _ekBedrijfCtx(b);
    if (!(ctx.afname > 0)) return res.status(400).json({ error: 'afname_mwh (jaarverbruik) verplicht' });
    const _n0 = (v) => { const x = Number(v); return (isFinite(x) && x > 0) ? x : 0; };
    const pv = Math.round(_n0(b.pv_kwp)), bkw = Math.round(_n0(b.batt_kw));
    const t0 = Date.now();
    const rap = await _ekBedrijfRapport(ctx, pv, bkw);
    console.log(`[bedrijf-rapport] pv ${pv} kWp · batt ${bkw} kW → besparing ${rap.factuur.besparing_jaar} · ${rap.cycli || 0} cycli/j in ${Date.now() - t0}ms`);
    return res.json(Object.assign({ ok: true }, rap, { _meta: { server_version: SERVER_VERSIE, elapsed_ms: Date.now() - t0 } }));
  } catch (e) { console.error('[bedrijf-rapport] faalde:', e.message); return res.status(500).json({ error: e.message }); }
});
...
app.post('/api/kamino/profiel-upload', async (req, res) => {
  ...
  try {
    ...
    // v15.157 (fase 2): gemeten afname-profiel binnen → probeer de "simulatie klaar"-mail te triggeren (guards binnenin).
    let rapport2 = null;
    if (type === 'afname' && rec.lead_token) { try { rapport2 = await _rapport2Trigger(rec.lead_token, 'profiel-upload'); } catch (e) {} }
    return res.json({ ok: true, type, bestand, profielen: rec.profielen, rapport2 });
  } catch (e) {
...
// ─── v15.159.0 — PROFIEL-IMPORT (AI-formaatherkenning → gemeten profiel) — MANAGER-ONLY, ADDITIEF ─────────────
// Portal-tegel apps/profiel-import.html. De kern (profiel/import.js) is puur en herbruikbaar voor de EK-intake.
//   POST /api/profiel/analyse  { project_id, afname:<bestand>, injectie?:<bestand>, slp_profiel?, pv_kwp?, omvormer_kva? }
//        <bestand> = { naam, soort:'csv'|'rows', gz_b64? | tekst? | rows? }  (Excel wordt in de browser naar rijen geparst)
//        → formaat (builtin / registry / AI / heuristiek), conversie, aanvulling + herkomst, coverage + preview.
//          Bewaart ENKEL een concept (profiel_import/concept/<id>.json) — niets definitief.
//   POST /api/profiel/bevestig { concept_id, forceer?, ean?, klant? }
//        → schrijft rec.profielen[type] in het COMPATIBELE formaat (canoniek 35040-kwartier-CSV, zodat
//          _opgeladenProfiel/_pasOpgeladen… het ongewijzigd oppikken) + sidecar-JSON (kwh + herkomst + coverage)
//          + historie (vorige meta, max 10). Een slechtere upload (minder gemeten én niet recenter) → 409 tenzij forceer.
//   GET  /api/profiel/registry → gekende converter-specs (converters/registry.json).
// De AI krijgt ENKEL een sample en levert een declaratieve spec; de transformatie is deterministisch. Nooit AI-code.
// Elke fout → foutmelding in de tegel; er wordt niets opgeslagen → de sims blijven op het bestaande gedrag (SLP / vorig profiel).
const profielImport = require('./profiel/import');
const _zlib = require('zlib');
const _PI_FP_RE = /^[0-9a-f]{16}$/;
const _PI_CID_RE = /^[0-9a-f]{24}$/;
async function _piJsonLees(pad) { try { return JSON.parse(await _factuurDownload(pad, true)); } catch (e) { return null; } }
async function _piJsonSchrijf(pad, obj) { await _factuurUpload(Buffer.from(JSON.stringify(obj), 'utf8').toString('base64'), 'application/json', pad); }
async function _piRegistryLees() { const r = await _piJsonLees('converters/registry.json'); return (r && r.formaten) ? r : { versie: 1, formaten: {} }; }
async function _piRegistryZet(fp, patch, specDoc) {
  if (!_PI_FP_RE.test(fp)) return;
  try {
    if (specDoc) await _piJsonSchrijf(`converters/${fp}.json`, specDoc);
    const reg = await _piRegistryLees();
    const cur = reg.formaten[fp] || { aangemaakt: new Date().toISOString(), gebruik: 0, bevestigd: false };
    reg.formaten[fp] = Object.assign(cur, patch || {}, { laatst: new Date().toISOString() });
    reg.bijgewerkt = new Date().toISOString();
    await _piJsonSchrijf('converters/registry.json', reg);
  } catch (e) { console.warn('[profiel-import] registry bijwerken faalde (niet-blokkerend):', e.message); }
}
function _piDecodeBestand(b) {
  if (!b || typeof b !== 'object') throw new Error('bestand ontbreekt');
  const naam = String(b.naam || 'bestand').slice(0, 120);
  let payload = null;
  if (b.gz_b64) {
    const raw = _zlib.gunzipSync(Buffer.from(String(b.gz_b64), 'base64'), { maxOutputLength: 60 * 1024 * 1024 });
    payload = raw.toString('utf8');
  } else if (typeof b.tekst === 'string') payload = b.tekst;
  if (b.soort === 'rows') {
    const rows = Array.isArray(b.rows) ? b.rows : JSON.parse(payload || '[]');
    if (!Array.isArray(rows) || !rows.length) throw new Error(`${naam}: geen rijen`);
    if (rows.length > 600000) throw new Error(`${naam}: te veel rijen (${rows.length})`);
    return { naam, kind: 'rows', delimiter: null, rows: rows.map(r => Array.isArray(r) ? r.slice(0, 60).map(c => (typeof c === 'number' ? c : (c == null ? '' : String(c).slice(0, 200)))) : []) };
  }
  if (typeof payload !== 'string' || !payload.trim()) throw new Error(`${naam}: leeg bestand`);
  const p = profielImport.parseCsv(payload);
  if (p.rows.length > 600000) throw new Error(`${naam}: te veel rijen (${p.rows.length})`);
  return { naam, kind: 'csv', delimiter: p.delimiter, rows: p.rows };
}
// Zoekt/bouwt de conversie-spec: (manuele override →) builtin → registry → AI (max 2 pogingen) → heuristiek. Test elke kandidaat op het VOLLEDIGE bestand.
async function _piSpecVoor(bestand, rol, override) {
  const sn = profielImport.sniff(bestand.rows, bestand.kind, bestand.delimiter);
  const fp = profielImport.fingerprint(sn);
  const pogingen = [];
  const probeer = (kandidaat, bron) => {
    const v = profielImport.validateSpec(kandidaat, sn);
    if (!v.ok) { pogingen.push({ bron, fout: v.fout, spec: kandidaat }); return null; }
    const conv = profielImport.applySpec(bestand.rows, v.spec, rol);
    const san = profielImport.sanity(conv);
    if (!san.ok) { pogingen.push({ bron, fout: san.fout, spec: v.spec }); return null; }
    return { spec: v.spec, conv, san };
  };
  // (0) Adviseur-mapping: een expliciete, door de adviseur bevestigde kolom/reeks-toewijzing wint. Slaagt ze de
  //     controle op het volledige bestand → bron 'manueel' (geen AI-call). Faalt ze → we vallen terug op auto-detectie.
  if (override && typeof override === 'object') {
    const r = probeer(override, 'manueel');
    if (r) return Object.assign(r, { bron: 'manueel', fingerprint: fp, sniff: sn, nieuw: false, pogingen });
  }
  const b = profielImport.builtinSpec(sn);
  if (b) { const r = probeer(b.spec, 'builtin'); if (r) return Object.assign(r, { bron: 'builtin', fingerprint: fp, sniff: sn, nieuw: false, pogingen }); }
  const reg = await _piRegistryLees();
  if (reg.formaten[fp]) {
    const doc = await _piJsonLees(`converters/${fp}.json`);
    if (doc && doc.spec) { const r = probeer(doc.spec, 'registry'); if (r) return Object.assign(r, { bron: 'registry', fingerprint: fp, sniff: sn, nieuw: false, pogingen, registry: reg.formaten[fp] }); }
  }
  let feedback = null, aiFout = null;
  for (let i = 0; i < 2; i++) {
    try {
      const ai = await profielImport.aiSpec(bestand.rows, sn, rol, { feedback });
      const r = probeer(ai.spec, 'ai');
      if (r) {
        // kandidaat meteen registreren (bevestigd:false) → een volgend bestand in hetzelfde formaat hergebruikt de spec zonder AI
        if (!(reg.formaten[fp] && reg.formaten[fp].bevestigd)) {
          await _piRegistryZet(fp, { naam: r.spec.naam, bron: 'ai', bevestigd: false, headers: sn.header.slice(0, 40), model: ai.model },
            { fingerprint: fp, spec: r.spec, headers: sn.header.slice(0, 40), types: sn.types, aangemaakt: new Date().toISOString(), model: ai.model, versie: profielImport.VERSIE });
        }
        return Object.assign(r, { bron: 'ai', fingerprint: fp, sniff: sn, nieuw: true, pogingen, model: ai.model });
      }
      feedback = (pogingen[pogingen.length - 1] || {}).fout ? pogingen[pogingen.length - 1].fout.join('; ') : null;
      if (ai.spec && ai.spec.manuele_review) break;
    } catch (e) { aiFout = e.message; pogingen.push({ bron: 'ai', fout: [e.message] }); break; }
  }
  const h = profielImport.heuristicSpec(sn);
  if (h) { const r = probeer(h.spec, 'heuristiek'); if (r) return Object.assign(r, { bron: 'heuristiek', fingerprint: fp, sniff: sn, nieuw: false, pogingen, manuele_review_aanbevolen: true, ai_fout: aiFout }); }
  return { fout: 'Formaat niet automatisch te converteren — manuele review nodig.', fingerprint: fp, sniff: sn, pogingen };
}
function _piSlpNaam(rec, b) {
  const kand = [b.slp_profiel, rec && rec.profiel, rec && rec.input && (rec.input.profielNaam || rec.input.profiel)].find(x => x && typeof x === 'string');
  return kand ? String(kand).replace(/[^A-Za-z0-9 _\-\/]/g, '').trim().slice(0, 60) || 'kantoor' : 'kantoor';   // geen '.' → geen pad-traversal
}
function _piRond(arr) { return arr.map(v => (v > 0 ? Number(v.toPrecision(6)) : 0)); }

// v15.160: jaarverbruik-bron. Het profiel levert ALTIJD de vorm; zijn eigen jaartotaal wordt de jaarverbruik-default
// TENZIJ het profiel > drempel geëxtrapoleerd is ÉN er een echte jaarfactuur (periode ≥ 330 d, geen voorschot) op het
// project staat — dan blijft het factuurvolume leidend. (Johan 22-09.)
function _piDatum(s) {
  const t = String(s || '').trim(); let m;
  if ((m = t.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/))) return Date.UTC(+m[1], +m[2] - 1, +m[3]);
  if ((m = t.match(/^(\d{1,2})[-\/.](\d{1,2})[-\/.](\d{4})/))) return Date.UTC(+m[3], +m[2] - 1, +m[1]);
  return NaN;
}
function _piJaarfactuur(rec) {
  const bc = (rec && (rec.baseCase || (rec.input && rec.input.baseCase))) || null;
  if (!bc) return { jaarfactuur: false, dagen: null };
  const v = _piDatum(bc.periodeVan), t = _piDatum(bc.periodeTot);
  const dagen = (isFinite(v) && isFinite(t) && t > v) ? Math.round((t - v) / 86400000) + 1 : null;
  return { jaarfactuur: !!(dagen && dagen >= 330 && !bc.is_voorschot), dagen };
}
function _piJaarBron(rec, labelType) {
  const jf = _piJaarfactuur(rec);
  const bron = (labelType === 'gemeten' || !jf.jaarfactuur) ? 'profiel' : 'factuur';
  return { bron, jaarfactuur: jf.jaarfactuur, factuur_dagen: jf.dagen,
    uitleg: bron === 'profiel'
      ? (labelType === 'gemeten' ? 'profiel is ≥ 90% gemeten → zijn jaartotaal is het jaarverbruik' : 'geen jaarfactuur op het project → het (geëxtrapoleerde) jaartotaal van het profiel is het jaarverbruik')
      : `jaarfactuur (${jf.dagen} d) op het project en profiel > drempel geëxtrapoleerd → factuurvolume blijft het jaarverbruik, profiel levert de vorm` };
}

// ─── v15.160 — MANUELE REVIEW: een niet-converteerbaar bestand wordt NIET weggegooid ─────────────────────────────
// Het originele bestand + sample + de mislukte pogingen gaan naar profiel_import/review/<rid>*. In de tegel kan een
// manager het bestand downloaden, de conversie-spec (JSON) invullen/corrigeren en testen. Slaagt de spec op het
// bewaarde bestand → spec in de registry (bron 'manueel', bevestigd) en de analyse loopt verder (zelfde scherm).
// Volgende bestanden in hetzelfde formaat worden daarna automatisch herkend.
const _PI_RID_RE = /^[0-9a-f]{16}$/;
async function _piReviewIndex() { const r = await _piJsonLees('profiel_import/review/index.json'); return (r && Array.isArray(r.items)) ? r : { items: [] }; }
async function _piReviewIndexZet(item) {
  const idx = await _piReviewIndex();
  const kort = { rid: item.rid, ts: item.ts, status: item.status, project_id: item.project_id, rol: item.failing.rol, bestand: item.failing.bestand, reden: item.failing.reden, door: item.door, opgelost_ts: item.opgelost_ts || null };
  idx.items = [kort].concat(idx.items.filter(x => x.rid !== item.rid)).slice(0, 300);
  idx.bijgewerkt = new Date().toISOString();
  await _piJsonSchrijf('profiel_import/review/index.json', idx);
}
async function _piReviewMaak(u, pid, params, decoded, failing) {
  const rid = require('crypto').randomBytes(8).toString('hex');
  const files = [];
  for (const d of decoded) {
    const isRows = d.best.kind === 'rows';
    const pad = `profiel_import/review/${rid}_${d.rol}.${isRows ? 'json' : 'csv'}`;
    const payload = isRows ? JSON.stringify(d.best.rows) : d.tekst;
    await _factuurUpload(Buffer.from(payload, 'utf8').toString('base64'), isRows ? 'application/json' : 'text/csv', pad);
    files.push({ rol: d.rol, naam: d.best.naam, soort: isRows ? 'rows' : 'csv', pad });
  }
  const fb = decoded.find(d => d.rol === failing.rol);
  const sn = failing.sniff;
  const sugg = ((failing.pogingen || []).slice().reverse().find(p => p.spec) || {}).spec || ((profielImport.heuristicSpec(sn) || {}).spec) || null;
  const item = { rid, ts: new Date().toISOString(), status: 'open', project_id: pid, door: u.email || u.id || null,
    params: { slp_profiel: params.slp_profiel || null, pv_kwp: params.pv_kwp || null, omvormer_kva: params.omvormer_kva || null },
    files, failing: { rol: failing.rol, bestand: fb ? fb.best.naam : '', reden: failing.reden, fingerprint: failing.fingerprint,
      header: sn ? sn.header : [], types: sn ? sn.types : [], delimiter: sn ? sn.delimiter : null, decimal: sn ? sn.decimal : null,
      has_header: sn ? sn.has_header : null, skip_rows: sn ? sn.skip_rows : null, ncol: sn ? sn.ncol : null,
      sample: fb ? profielImport.buildSampleText(fb.best.rows, sn) : '', pogingen: (failing.pogingen || []).map(p => ({ bron: p.bron, fout: p.fout })) },
    suggestie: sugg };
  await _piJsonSchrijf(`profiel_import/review/${rid}.json`, item);
  await _piReviewIndexZet(item);
  console.log(`[profiel/review] nieuw item ${rid} (${pid}, ${failing.rol}: ${item.failing.bestand}) — ${failing.reden}`);
  return item;
}

// Kern van de analyse (gedeeld door /api/profiel/analyse en de review-flow). Retourneert { status, body }.
async function _piAnalyseKern(u, b, ctx) {
  ctx = ctx || {};
  const pid = String(b.project_id || '').trim().toUpperCase();
  if (!/^FLX-[A-Z0-9]{3}-[A-Z0-9]{3,4}$/.test(pid)) return { status: 400, body: { error: 'geldig project-id verplicht' } };
  if (!b.afname) return { status: 400, body: { error: 'afname-bestand is verplicht' } };
  const t0 = Date.now();
  const { veilig, rec } = await _kaminoRecLaden(pid);
  if (!rec) return { status: 404, body: { error: 'Geen project gevonden met dit nummer.' } };
  const bestanden = [{ rol: 'afname', b: b.afname }].concat(b.injectie ? [{ rol: 'injectie', b: b.injectie }] : []);
  const decoded = [];
  for (const x of bestanden) {
    try {
      const best = _piDecodeBestand(x.b);
      let tekst = null;
      if (best.kind === 'csv') tekst = x.b.gz_b64 ? _zlib.gunzipSync(Buffer.from(String(x.b.gz_b64), 'base64'), { maxOutputLength: 60 * 1024 * 1024 }).toString('utf8') : x.b.tekst;
      decoded.push({ rol: x.rol, best, tekst });
    } catch (e) { return { status: 400, body: { error: e.message, rol: x.rol } }; }
  }
  const formaten = [], convs = {};
  const naarReview = async (failing) => {
    if (ctx.geenReview) return null;
    try { return await _piReviewMaak(u, pid, b, decoded, failing); } catch (e) { console.warn('[profiel/review] bewaren faalde:', e.message); return null; }
  };
  const overrides = (b.specs && typeof b.specs === 'object') ? b.specs : {};
  for (const d of decoded) {
    const r = await _piSpecVoor(d.best, d.rol, overrides[d.rol] || null);
    if (r.fout) {
      const item = await naarReview({ rol: d.rol, reden: r.fout, fingerprint: r.fingerprint, sniff: r.sniff, pogingen: r.pogingen });
      return { status: 422, body: { error: r.fout, rol: d.rol, bestand: d.best.naam, manuele_review: true, review_id: item ? item.rid : null,
        fingerprint: r.fingerprint, header: r.sniff.header, pogingen: r.pogingen.map(p => ({ bron: p.bron, fout: p.fout })) } };
    }
    formaten.push({ rol: d.rol, bestand: d.best.naam, fingerprint: r.fingerprint, bron: r.bron, nieuw: r.nieuw, spec_naam: r.spec.naam, spec: r.spec,
      manuele_review_aanbevolen: !!r.manuele_review_aanbevolen, resolutie_min: r.conv.R, parse_rate: r.san.parse_rate, rijen: r.conv.rijen,
      waarschuwingen: r.conv.waarschuwingen.concat(r.conv.negatief_genegeerd ? [`${r.conv.negatief_genegeerd} negatieve afname-waarden op 0 gezet`] : []),
      bevat: ['afname', 'injectie'].filter(t => r.conv[t]), model: r.model || null });
    convs[d.rol] = { conv: r.conv, sniff: r.sniff, fingerprint: r.fingerprint };
  }
  // samenvoegen: afname uit het afnamebestand; injectie uit het injectiebestand, anders uit het afnamebestand (bv. Fluvius-registers)
  const conv = { afname: convs.afname.conv.afname, injectie: (convs.injectie && convs.injectie.conv.injectie) || convs.afname.conv.injectie || null };
  if (!conv.afname) {
    const reden = 'Geen afname gevonden in het afname-bestand (bevat enkel injectie?).';
    const item = await naarReview({ rol: 'afname', reden, fingerprint: convs.afname.fingerprint, sniff: convs.afname.sniff, pogingen: [] });
    return { status: 422, body: { error: reden, manuele_review: true, review_id: item ? item.rid : null } };
  }
  const slpNaam = _piSlpNaam(rec, b);
  let slp = _laadProfielKwartier(slpNaam);
  if (!(Array.isArray(slp) && slp.length === 35040)) slp = (MARKT && MARKT.profiel && MARKT.profiel.length === 35040) ? MARKT.profiel : null;
  const pvVorm = (MARKT && MARKT.solar_norm && MARKT.solar_norm.length === 35040) ? MARKT.solar_norm : null;
  const opts = {
    pv_kwp: Number(b.pv_kwp) > 0 ? Number(b.pv_kwp) : (rec.pv && Number(rec.pv.kwp) > 0 ? Number(rec.pv.kwp) : 0),
    omvormer_kva: Number(b.omvormer_kva) > 0 ? Number(b.omvormer_kva) : 0,
  };
  const rc = profielImport.reconstrueer(conv, slp, pvVorm, opts);
  const cid = require('crypto').randomBytes(12).toString('hex');
  const types = {}, concTypes = {};
  for (const t of Object.keys(rc.types)) {
    const x = rc.types[t];
    types[t] = { coverage: x.coverage, preview: profielImport.preview(x.kwh, x.herkomst) };
    concTypes[t] = { kwh: _piRond(x.kwh), herkomst: x.herkomst, coverage: x.coverage };
  }
  const concept = { concept_id: cid, project_id: pid, veilig, door: u.email || u.id || null, ts: new Date().toISOString(), versie: profielImport.VERSIE,
    slp_profiel: slpNaam, pv_opts: opts, formaten, energiebalans: rc.energiebalans, types: concTypes, review_id: ctx.review_id || null };
  await _piJsonSchrijf(`profiel_import/concept/${cid}.json`, concept);
  const huidig = {};
  for (const t of ['afname', 'injectie']) {
    const m = rec.profielen && rec.profielen[t];
    if (m && m.bestand) huidig[t] = { bestand: m.bestand, op: m.op || null, van: m.van || null, tot: m.tot || null, mwh: m.mwh, label: (m.import && m.import.label) || 'opgeladen (Fluvius-converter)', pct_gemeten: m.import ? m.import.pct_gemeten : null };
  }
  const jaarverbruik = _piJaarBron(rec, types.afname.coverage.label_type);
  console.log(`[profiel/analyse] ${pid}: ${formaten.map(f => f.rol + '=' + f.bron + '/' + f.fingerprint).join(', ')} → afname ${types.afname.coverage.label} (${types.afname.coverage.pct_gemeten}% gemeten) · jaarverbruik=${jaarverbruik.bron} · ${Date.now() - t0}ms`);
  return { status: 200, body: { ok: true, concept_id: cid, project_id: pid, project_naam: rec.naam || (rec.klant && (rec.klant.naam || rec.klant.name)) || '',
    formaten: formaten.map(f => Object.assign({}, f, { spec: undefined, spec_kort: { unit: f.spec.unit, timezone: f.spec.timezone, label: f.spec.datetime.label, long_format: !!f.spec.long_format } })),
    types, energiebalans: rc.energiebalans, flags: rc.flags, slp_profiel: slpNaam, pv_opts: opts, huidig, jaarverbruik, herkomst_labels: profielImport.HERKOMST_LABELS,
    review_id: ctx.review_id || null, elapsed_ms: Date.now() - t0 } };
}

app.post('/api/profiel/analyse', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  try { const r = await _piAnalyseKern(u, req.body || {}); return res.status(r.status).json(r.body); }
  catch (e) { console.error('[profiel/analyse] fout:', e.message); return res.status(500).json({ error: e.message }); }
});

// POST /api/profiel/detect  { bestand:{naam,soort,tekst|gz_b64|rows} }  → STATELOZE inspectie (geen project, niets bewaard).
// Herkent het formaat (builtin/registry) of stelt er één voor (heuristiek), en identificeert de MEETREEKSEN (numerieke
// kolommen) met een voorgestelde rol, zodat de adviseur in de EK-prep-tegel kan aangeven welke reeks afname en welke
// injectie is. Geen AI-call — snel en interactief. De adviseur-mapping gaat daarna als `specs` mee naar /api/profiel/analyse.
app.post('/api/profiel/detect', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  try {
    const best = _piDecodeBestand((req.body || {}).bestand);
    const sn = profielImport.sniff(best.rows, best.kind, best.delimiter);
    const fp = profielImport.fingerprint(sn);
    // Voorstel-spec: builtin → registry (bevestigd/AI) → heuristiek. Geen AI.
    let voorstel = null, bron = null, herkend = false, spec_naam = null;
    const bi = profielImport.builtinSpec(sn);
    if (bi) { voorstel = bi.spec; bron = 'builtin'; herkend = true; spec_naam = bi.spec.naam; }
    if (!voorstel) {
      const reg = await _piRegistryLees();
      if (reg.formaten[fp]) {
        const doc = await _piJsonLees(`converters/${fp}.json`);
        if (doc && doc.spec) { voorstel = doc.spec; bron = reg.formaten[fp].bron || 'registry'; herkend = true; spec_naam = reg.formaten[fp].naam || doc.spec.naam; }
      }
    }
    if (!voorstel) { const h = profielImport.heuristicSpec(sn); if (h) { voorstel = h.spec; bron = 'heuristiek'; spec_naam = h.spec.naam; } }
    const v = voorstel ? profielImport.validateSpec(voorstel, sn) : null;
    const spec = v && v.ok ? v.spec : (voorstel || null);
    // Meetreeksen = numerieke kolommen (breed formaat) of long_format-sleutels.
    const hdr = sn.header || [];
    const isLong = !!(spec && spec.long_format);
    const rolVan = (idx) => {
      if (!spec) return null;
      if (spec.value_columns && Array.isArray(spec.value_columns.afname) && spec.value_columns.afname.includes(idx)) return 'afname';
      if (spec.value_columns && Array.isArray(spec.value_columns.injectie) && spec.value_columns.injectie.includes(idx)) return 'injectie';
      return null;
    };
    const reeksen = [];
    for (let c = 0; c < sn.ncol; c++) {
      if (sn.types[c] !== 'n') continue;   // enkel numerieke kolommen zijn meetreeksen
      reeksen.push({ idx: c, naam: (hdr[c] != null && String(hdr[c]).trim()) ? String(hdr[c]).slice(0, 60) : ('kolom ' + c), rol: rolVan(c) });
    }
    // Long-format: de reeksen zitten in rijen (register-sleutel), niet in kolommen → geef de sleutels terug.
    const long_reeksen = isLong ? {
      key_col: spec.long_format.key_col, value_col: spec.long_format.value_col,
      afname: spec.long_format.afname || [], injectie: spec.long_format.injectie || [], match: spec.long_format.match,
    } : null;
    return res.json({ ok: true, fingerprint: fp, herkend, bron, spec_naam,
      header: hdr.slice(0, 40), types: sn.types, ncol: sn.ncol, delimiter: sn.delimiter, decimal: sn.decimal,
      has_header: sn.has_header, skip_rows: sn.skip_rows,
      sample: profielImport.buildSampleText(best.rows, sn),
      long_format: isLong, long_reeksen, reeksen,
      voorstel_spec: spec, module_versie: profielImport.VERSIE });
  } catch (e) { console.error('[profiel/detect] fout:', e.message); return res.status(400).json({ error: e.message }); }
});

// GET /api/profiel/review(?alle=1) → lijst (default enkel 'open')
app.get('/api/profiel/review', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  try { const idx = await _piReviewIndex(); const items = req.query.alle ? idx.items : idx.items.filter(x => x.status === 'open'); return res.json({ ok: true, items }); }
  catch (e) { return res.status(500).json({ error: e.message }); }
});
// GET /api/profiel/review/:rid → detail + download-links (signed, 10 min)
app.get('/api/profiel/review/:rid', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  const rid = String(req.params.rid || '').toLowerCase(); if (!_PI_RID_RE.test(rid)) return res.status(400).json({ error: 'ongeldig review-id' });
  try {
    const item = await _piJsonLees(`profiel_import/review/${rid}.json`); if (!item) return res.status(404).json({ error: 'review niet gevonden' });
    for (const f of item.files) { try { f.download_url = await _factuurSignedUrl(f.pad, 600); } catch (e) { f.download_url = null; } }
    return res.json(Object.assign({ ok: true }, item));
  } catch (e) { return res.status(500).json({ error: e.message }); }
});
// POST /api/profiel/review/:rid/spec { spec } → test de (manueel ingevulde) spec op het bewaarde bestand; OK → registry
// (bron 'manueel', bevestigd) + de volledige analyse loopt verder (zelfde antwoord als /api/profiel/analyse, niets definitief).
app.post('/api/profiel/review/:rid/spec', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  const rid = String(req.params.rid || '').toLowerCase(); if (!_PI_RID_RE.test(rid)) return res.status(400).json({ error: 'ongeldig review-id' });
  try {
    const item = await _piJsonLees(`profiel_import/review/${rid}.json`); if (!item) return res.status(404).json({ error: 'review niet gevonden' });
    const spec = (req.body || {}).spec;
    // bewaarde bestanden terug inlezen
    const body = Object.assign({ project_id: item.project_id }, item.params || {});
    for (const f of item.files) {
      const txt = await _factuurDownload(f.pad, true);
      body[f.rol] = f.soort === 'rows' ? { naam: f.naam, soort: 'rows', rows: JSON.parse(txt) } : { naam: f.naam, soort: 'csv', tekst: txt };
    }
    const fb = body[item.failing.rol];
    const best = _piDecodeBestand(fb);
    const sn = profielImport.sniff(best.rows, best.kind, best.delimiter);
    const fp = profielImport.fingerprint(sn);
    const v = profielImport.validateSpec(spec, sn);
    if (!v.ok) return res.status(422).json({ error: 'Spec ongeldig', fout: v.fout });
    const conv = profielImport.applySpec(best.rows, v.spec, item.failing.rol);
    const san = profielImport.sanity(conv);
    if (!san.ok) return res.status(422).json({ error: 'Spec faalt de controle op het volledige bestand', fout: san.fout,
      test: { rijen: conv.rijen, geparst: conv.geparst, resolutie_min: conv.R, afname: !!conv.afname, injectie: !!conv.injectie } });
    v.spec.manuele_review = false;
    await _piRegistryZet(fp, { naam: v.spec.naam, bron: 'manueel', bevestigd: true, headers: sn.header.slice(0, 40), bevestigd_door: u.email || u.id || null, review_id: rid },
      { fingerprint: fp, spec: v.spec, headers: sn.header.slice(0, 40), types: sn.types, aangemaakt: new Date().toISOString(), bron: 'manueel', review_id: rid, versie: profielImport.VERSIE });
    const r = await _piAnalyseKern(u, body, { geenReview: true, review_id: rid });
    if (r.status === 200) { item.status = 'opgelost'; item.opgelost_ts = new Date().toISOString(); item.opgelost_door = u.email || u.id || null; item.spec = v.spec; item.fingerprint_opgelost = fp; }
    else { item.laatste_poging = { ts: new Date().toISOString(), fout: r.body.error, rol: r.body.rol || null }; }
    await _piJsonSchrijf(`profiel_import/review/${rid}.json`, item); await _piReviewIndexZet(item);
    return res.status(r.status).json(r.body);
  } catch (e) { console.error('[profiel/review] fout:', e.message); return res.status(500).json({ error: e.message }); }
});
// POST /api/profiel/review/:rid/afwijzen { reden? } → item sluiten zonder conversie (bestand blijft bewaard)
app.post('/api/profiel/review/:rid/afwijzen', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  const rid = String(req.params.rid || '').toLowerCase(); if (!_PI_RID_RE.test(rid)) return res.status(400).json({ error: 'ongeldig review-id' });
  try {
    const item = await _piJsonLees(`profiel_import/review/${rid}.json`); if (!item) return res.status(404).json({ error: 'review niet gevonden' });
    item.status = 'afgewezen'; item.opgelost_ts = new Date().toISOString(); item.opgelost_door = u.email || u.id || null; item.afwijs_reden = String((req.body || {}).reden || '').slice(0, 300);
    await _piJsonSchrijf(`profiel_import/review/${rid}.json`, item); await _piReviewIndexZet(item);
    return res.json({ ok: true });
  } catch (e) { return res.status(500).json({ error: e.message }); }
});

// "Tot een betere input": nieuw wint bij meer gemeten kwartieren, of recenter venster met ≥ 80% van het gemeten volume.
function _piWint(nieuw, oud) {
  if (!oud || !oud.bestand) return { wint: true, reden: 'geen bestaand profiel' };
  const gemOud = oud.import ? Number(oud.import.gemeten_kwartieren) || 0 : Number(oud.n_kwartier) || 0;
  const gemNieuw = Number(nieuw.gemeten_kwartieren) || 0;
  const pd = s => { const m = String(s || '').match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/); return m ? Date.UTC(+m[3], +m[2] - 1, +m[1]) : 0; };
  const totOud = pd(oud.tot), totNieuw = pd(nieuw.tot);
  if (!gemOud) return { wint: true, reden: 'bestaand profiel zonder dekkingsinfo' };
  if (gemNieuw >= gemOud) return { wint: true, reden: 'meer (of evenveel) gemeten kwartieren' };
  if (totNieuw > totOud && gemNieuw >= 0.8 * gemOud) return { wint: true, reden: 'recenter venster' };
  return { wint: false, reden: `minder gemeten (${gemNieuw} < ${gemOud} kwartieren) en niet recenter`, gem_oud: gemOud, gem_nieuw: gemNieuw };
}

app.post('/api/profiel/bevestig', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  const b = req.body || {};
  const cid = String(b.concept_id || '').toLowerCase();
  if (!_PI_CID_RE.test(cid)) return res.status(400).json({ error: 'geldig concept_id verplicht' });
  try {
    const c = await _piJsonLees(`profiel_import/concept/${cid}.json`);
    if (!c || !c.types || !c.types.afname) return res.status(404).json({ error: 'concept niet gevonden (opnieuw analyseren)' });
    const veilig = String(c.project_id).replace(/[^A-Za-z0-9._-]/g, '_').slice(0, 40);
    const rec = await _piJsonLees(`kamino/${veilig}.json`);   // VERSE read (cache-buster) — nooit een stale record terugschrijven
    if (!rec) return res.status(404).json({ error: 'project niet gevonden' });
    rec.profielen = rec.profielen || {};
    const conflicten = {};
    for (const t of Object.keys(c.types)) {
      const w = _piWint(c.types[t].coverage, rec.profielen[t]);
      if (!w.wint) conflicten[t] = w;
    }
    if (Object.keys(conflicten).length && !b.forceer) return res.status(409).json({ conflict: true, conflicten, error: 'Het bestaande profiel is beter gedekt. Bevestig met "toch gebruiken" om te overschrijven (vorige versie blijft in de historie).' });
    const veiligS = (s, fb) => String(s || fb).replace(/[^A-Za-z0-9._-]/g, '_').slice(0, 60);
    const stamp = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 12);
    const bewaard = {};
    for (const t of Object.keys(c.types)) {
      const x = c.types[t], cov = x.coverage, prev = rec.profielen[t] || null;
      const eanRaw = b.ean || (prev && prev.ean) || null;
      const klantRaw = b.klant || (prev && prev.klant) || (rec.klant && (rec.klant.naam || rec.klant.name)) || rec.naam || null;
      const basis = `profielen/${veilig}_${veiligS(eanRaw, 'geenEAN')}_${veiligS(klantRaw, 'klant')}_${t}_imp${stamp}`;
      await _factuurUpload(Buffer.from(profielImport.toCanonicalCsv(x.kwh), 'utf8').toString('base64'), 'text/csv', basis + '.csv');
      const fmt = (c.formaten || []).filter(f => f.bevat && f.bevat.includes(t));
      await _piJsonSchrijf(basis + '.json', { versie: profielImport.VERSIE, project_id: c.project_id, type: t, kwh: x.kwh, herkomst: x.herkomst,
        herkomst_labels: profielImport.HERKOMST_LABELS, coverage: cov, formaten: fmt.map(f => ({ bestand: f.bestand, fingerprint: f.fingerprint, bron: f.bron, spec: f.spec })),
        energiebalans: c.energiebalans, slp_profiel: c.slp_profiel, concept_id: cid });
      const historie = prev ? [Object.assign({}, prev, { historie: undefined })].concat(Array.isArray(prev.historie) ? prev.historie : []).slice(0, 10) : [];
      rec.profielen[t] = {
        bestand: basis + '.csv', ean: eanRaw, klant: klantRaw,
        mwh: cov.jaar_mwh, piek_kw: cov.maandpiek_kw, van: cov.van, tot: cov.tot, n_kwartier: cov.gemeten_kwartieren,
        door: u.email || u.id || null, op: new Date().toISOString(),
        import: {
          versie: profielImport.VERSIE, json: basis + '.json', label: cov.label, label_type: cov.label_type, drempel_pct: cov.drempel_pct,
          pct_gemeten: cov.pct_gemeten, pct_afgeleid: cov.pct_afgeleid,
          methodes: { uurresolutie: cov.pct_uurresolutie, interpolatie: cov.pct_interpolatie, typische_dag: cov.pct_typische_dag, slp: cov.pct_slp, energiebalans: cov.pct_energiebalans },
          gemeten_kwartieren: cov.gemeten_kwartieren, gemeten_span: cov.gemeten_span, gemeten_maanden: cov.gemeten_maanden,
          maandpiek_kw_gemeten: cov.maandpiek_kw_gemeten, flags: cov.flags || [],
          formaat: fmt.map(f => ({ fingerprint: f.fingerprint, bron: f.bron, naam: f.spec_naam })), slp_profiel: c.slp_profiel,
          energiebalans: c.energiebalans || null, concept_id: cid, geforceerd: !!conflicten[t], review_id: c.review_id || null,
          jaarverbruik_bron: t === 'afname' ? _piJaarBron(rec, cov.label_type).bron : null,
        },
        historie,
      };
      bewaard[t] = { bestand: basis + '.csv', label: cov.label };
    }
    rec.bijgewerkt = new Date().toISOString();
    await _piJsonSchrijf(`kamino/${veilig}.json`, rec);
    try { _kaminoLijstCache.ts = 0; } catch (e) {}
    for (const f of (c.formaten || [])) {   // converter-registry: bevestigd + gebruik (builtins hoeven niet)
      if (f.bron === 'builtin' || !_PI_FP_RE.test(f.fingerprint || '')) continue;
      const reg = await _piRegistryLees(); const cur = reg.formaten[f.fingerprint] || {};
      const doc = f.bron !== 'registry' ? { fingerprint: f.fingerprint, spec: f.spec, aangemaakt: new Date().toISOString(), bron: f.bron, versie: profielImport.VERSIE } : null;
      await _piRegistryZet(f.fingerprint, { naam: f.spec_naam, bron: f.bron === 'registry' ? (cur.bron || 'registry') : f.bron, bevestigd: true,
        gebruik: (cur.gebruik || 0) + 1, bevestigd_door: u.email || u.id || null }, doc);
    }
    console.log(`[profiel/bevestig] ${c.project_id}: ${Object.entries(bewaard).map(([t, x]) => `${t} → ${x.bestand} (${x.label})`).join(' · ')}`);
    let rapport2 = null;
    if (bewaard.afname && rec.lead_token) { try { rapport2 = await _rapport2Trigger(rec.lead_token, 'profiel-import'); } catch (e) {} }
    return res.json({ ok: true, project_id: c.project_id, bewaard, profielen: rec.profielen, rapport2 });
  } catch (e) {
    console.error('[profiel/bevestig] fout:', e.message);
    return res.status(500).json({ error: e.message });
  }
});

app.get('/api/profiel/registry', async (req, res) => {
  const u = await _managerGuard(req, res); if (!u) return;
  try { return res.json(Object.assign({ ok: true, module_versie: profielImport.VERSIE }, await _piRegistryLees())); }
  catch (e) { return res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    for (const rec of recs) {
      ...
      out.push({
        ...
        offertes_aangevraagd: !!rec.offertes_aangevraagd,                 // v15.156.8: klant vroeg offertes aan (→ minibestek naar partners)
      });
    }
    out.sort((a, b) => String(b.bijgewerkt || '').localeCompare(String(a.bijgewerkt || '')));
...
app.post('/api/kamino/archiveer', async (req, res) => {
  ...
  try {
    ...
    await _factuurUpload(Buffer.from(JSON.stringify(rec), 'utf8').toString('base64'), 'application/json', `kamino/${veilig}.json`);
    try { _kaminoLijstCache.ts = 0; } catch (e) {}
    // v15.157.3: archief-vlag ook op de gekoppelde lead → een gearchiveerde lead wordt overgeslagen in de EAN+klant-
    //   matching (nieuwe factuur = NIEUW project). De-archiveren zet ze terug actief zodat een nieuwe factuur weer
    //   updatet op dit dossier.
    if (rec.lead_token) { try { const lr = _leadLezen(rec.lead_token); if (lr) { lr.archief = !!b.archived; _leadOpslaan(rec.lead_token, lr); } } catch (e) {} }
...
app.get('/api/mandaat/wachtrij', async (req, res) => {
  ...
  try {
    ...
    const inclArchief = String(req.query.archief || '') === '1';   // v15.158: standaard verbergen we acties van gearchiveerde projecten
    let lijst = [];
    try { lijst = await _bucketList('kamino/'); } catch (e) { lijst = []; }
    const jsons = (Array.isArray(lijst) ? lijst : []).filter(o => o.name && /\.json$/i.test(o.name));
    const recs = await Promise.all(jsons.map(o =>
      _factuurDownload(`kamino/${o.name}`, true).then(t => { try { return JSON.parse(t); } catch (e) { return null; } }).catch(() => null)));
    // v15.158: gearchiveerde projecten (+ hun gekoppelde lead) → hun mandaat-acties vallen weg. We onthouden hun
    // project-id én lead_token zodat óók losse self-service-EAN's die naar zo'n dossier verwijzen worden weggefilterd.
    const archProjIds = new Set(), archLeadTokens = new Set();
    for (const rec of recs) { if (rec && rec.archief) { if (rec.id) archProjIds.add(String(rec.id)); if (rec.lead_token) archLeadTokens.add(String(rec.lead_token)); } }
    const out = [];
    for (const rec of recs) {
      if (!rec || !rec.id || !rec.mandaat || !Array.isArray(rec.mandaat.eans)) continue;
      if (rec.archief && !inclArchief) continue;   // gearchiveerd dossier → geen openstaande acties meer tonen
    ...
    try {
      ...
      for (const en of (los && los.eans) || []) {
        if (gevraagd.indexOf(en.status) < 0) continue;
        if (!inclArchief) {   // v15.158: losse EAN gekoppeld aan een gearchiveerd project/lead → ook weg
          const _pid = en.kamino_project_id ? String(en.kamino_project_id) : null;
          const _lt = en.lead_token ? String(en.lead_token) : null;
          if ((_pid && archProjIds.has(_pid)) || (_lt && archLeadTokens.has(_lt))) continue;
        }
...
app.post('/api/kamino/onderhandel', async (req, res) => {
  try {
    ...
    if (_pInfo) out.profiel_bron = { bron: 'opgeladen_afname', ean: _pInfo.ean, mwh: _pInfo.mwh, maanden: _pInfo.maanden, van: _pInfo.van, tot: _pInfo.tot, label: _pInfo.label || null };
    ...
    // zodat de Kamino-tegel-1-respons licht blijft. De Energie-Compas-onderhandelingsnota zet b.heatmaps=true.
...
app.post('/api/nominatie-sim', async (req, res) => {
  ...
  proc.on('close', code => {
    ...
    if (_piNom) result.profiel_bron = { bron:'opgeladen_afname', ean:_piNom.ean, mwh:_piNom.mwh, maanden:_piNom.maanden, maandpiek_kw:_piNom.maandpiek_kw, label:_piNom.label||null };   // v15.70 label
    res.json(result);
  });
...
async function _opgeladenProfiel(projectId, type) {
  ...
  // v15.159: geïmporteerd profiel (profiel/import.js) — het canonieke CSV beslaat altijd 12 kalendermaanden; geef de
  // GEMETEN maanden/venster + het coverage-label door. Zo blijft buildSimInput bij < 12 gemeten maanden op het factuur-
  // volume (enkel de vorm komt uit het profiel) en triggert het toegangsvermogen-advies (≥ 6 mnd) niet op een
  // gereconstrueerde piek. Legacy-profielen (zonder meta.import) → ongewijzigd.
  if (val && meta.import && typeof meta.import === 'object') {
    const gm = Number(meta.import.gemeten_maanden);
    if (isFinite(gm) && gm >= 0) val.maanden = gm;
    if (meta.van) val.van = meta.van;
    if (meta.tot) val.tot = meta.tot;
    val.label = meta.import.label || null;
    val.label_type = meta.import.label_type || null;
    val.pct_gemeten = meta.import.pct_gemeten != null ? meta.import.pct_gemeten : null;
    // v15.160: jaarverbruik-bron (zie _piJaarBron): profiel-totaal leidend, behalve bij > drempel extrapolatie + jaarfactuur.
    const _jb = _piJaarBron(rec, meta.import.label_type);
    val.jaar_authoritatief = _jb.bron === 'profiel';
    val.jaarverbruik_bron = _jb.bron;
    if (meta.import.label_type !== 'gemeten') { val.maandpiek_kw_gemeten = Number(meta.import.maandpiek_kw_gemeten) || 0; val.maandpiek_kw = 0; }   // geëxtrapoleerd → géén 12-mnd-piek (guards > 0 slaan het TV-advies over)
  }
...
async function _pasOpgeladenAfnameToe(input) {
  try {
    ...
    if (op && Array.isArray(op.kwartier) && op.kwartier.length === 35040) {
      ...
      const _jaarAuth = (op.jaar_authoritatief != null) ? !!op.jaar_authoritatief : (op.maanden >= 12);   // v15.160: geïmporteerd profiel → _piJaarBron; legacy ongewijzigd
      const _jaarMwh = (+op.jaar_mwh > 0 && _jaarAuth) ? +op.jaar_mwh : null;
      if (_jaarMwh) input._opgeladen_jaar_mwh = _jaarMwh;
      return { bron: 'opgeladen_afname', ean: op.ean, mwh: op.mwh, jaar_mwh: op.jaar_mwh || null, jaarverbruik_default_mwh: _jaarMwh, van: op.van, tot: op.tot, maandpiek_kw: op.maandpiek_kw, maanden: op.maanden, bestand: op.bestand, label: op.label || null, pct_gemeten: op.pct_gemeten != null ? op.pct_gemeten : null };   // v15.159: + label
    }
...
async function _pasOpgeladenInjectieToe(p) {
  try {
    ...
    if (op && Array.isArray(op.kwartier) && op.kwartier.length === 35040) {
      ...
      return { bron: 'opgeladen_injectie', ean: op.ean, mwh: op.mwh, van: op.van, tot: op.tot, maandpiek_kw: op.maandpiek_kw, maanden: op.maanden, bestand: op.bestand, label: op.label || null };   // v15.159: + label
    }
...
// ─── v15.110 (Fase 4 — Energie-Compas LEAD-CAPTURE / onderhandelingsnota per mail) ────────────
...
const WEB_BASE     = (process.env.WEB_BASE || 'https://app.energie-compas.eu').replace(/\/+$/, '');
...
async function _brevoMailRaw(to, subject, textContent, htmlContent, senderNaam, attachments) {
  if (!BREVO_API_KEY) return { sent: false, reden: 'BREVO_API_KEY niet gezet' };
  try {
    const body = { sender: { name: senderNaam || 'Energie-Compas', email: BREVO_SENDER },
      to: (Array.isArray(to) ? to : [to]).map(e => (typeof e === 'string' ? { email: e } : e)), subject, textContent };
    if (htmlContent) body.htmlContent = htmlContent;
    // v15.163: optionele bijlagen (Brevo: {name, content=base64}).
    if (Array.isArray(attachments) && attachments.length) body.attachment = attachments.filter(a => a && a.contentBytes).map(a => ({ name: String(a.name || 'bijlage.pdf'), content: String(a.contentBytes) }));
    const r = await fetch('https://api.brevo.com/v3/smtp/email', {
...
const _MAIL_TEMPLATES = {
  'rapport': 'Uw Energie-Compas-rapport (alle stadia — één template)',
...
// v15.157: KLANTMAIL-KANAAL voor de EK-flow. De EK-mails zijn REACTIES op de klant (factuur ontvangen → vul aan →
// simulatie klaar), geen abonnementsmails. Brevo zet op elke mail een List-Unsubscribe-header → Gmail toont dan
// "van een mailinglijst". Daarom versturen we deze klant-mails bij voorkeur via de EK-MAILBOX zelf (Microsoft Graph
// sendMail, dossier@energie-compas.eu): een échte reply uit de mailbox, zonder mailinglijst-banner. Valt terug op
// Brevo als Graph niet beschikbaar is of faalt. Back-office-notificaties blijven via _brevoMail (interne mail).
// v15.162.0: KANAALKEUZE via ENV EK_KLANTMAIL_KANAAL ('brevo' = default, of 'graph').
// Waarom Brevo default: uitgaande Graph-mail van het nieuwe M365-domein (dossier@energie-compas.eu) wordt
// door andere M365-tenants geweigerd (550 5.7.708 — nieuw-domein-reputatie). Brevo heeft warme verzend-IP's
// → betrouwbare aflevering naar M365-klanten vanaf dag één. Zet EK_KLANTMAIL_KANAAL=graph (+ restart) zodra
// de M365-reputatie is ingelopen (test na enkele dagen) voor de persoonlijkere Graph-look.
// In beide standen is het andere kanaal de fallback als het primaire faalt.
async function _ekKlantMail(to, subject, textContent, htmlContent, attachments) {
  const kanaal = (String(process.env.EK_KLANTMAIL_KANAAL || 'brevo').trim().toLowerCase() === 'graph') ? 'graph' : 'brevo';
  if (kanaal === 'graph' && graphInbound.graphEnabled()) {
    try { await graphInbound.sendMail(to, subject, htmlContent || null, textContent || '', attachments); return { sent: true, kanaal: 'graph' }; }
    catch (e) { console.warn('[ek-klantmail] Graph faalde, val terug op Brevo:', e.message); }
    const r = await _brevoMailRaw(to, subject, textContent, htmlContent, 'Energie-Compas', attachments);
    return { sent: !!r.sent, kanaal: 'brevo-fallback', reden: r.reden };
  }
  // Default: Brevo primair (betrouwbaar naar M365), Graph als noodval.
  const r = await _brevoMailRaw(to, subject, textContent, htmlContent, 'Energie-Compas', attachments);
  if (r && r.sent) return { sent: true, kanaal: 'brevo' };
  if (graphInbound.graphEnabled()) {
    try { await graphInbound.sendMail(to, subject, htmlContent || null, textContent || '', attachments); return { sent: true, kanaal: 'graph-fallback' }; }
    catch (e) { console.warn('[ek-klantmail] Brevo én Graph faalden:', e.message); }
  }
  return { sent: false, kanaal: 'brevo', reden: (r && r.reden) || '?' };
}
...
async function _seedKaminoProject(token, lead, km) {
  ...
  try {
    ...
    const nu = new Date().toISOString();
    const rec = {
      id, naam: lead.naam || 'Energie-Compas-lead',
      ...
      scenarios: { standaard: { scenario: 'standaard', baseCase: km.baseCase || null, input: km.input || null, bijgewerkt: nu, door: 'Energie-Compas' } },
      studies: {}, lead_token: token, samenvatting: lead.samenvatting || null,
      aangemaakt: nu, bijgewerkt: nu, door: 'Energie-Compas',
    };
    await _factuurUpload(Buffer.from(JSON.stringify(rec), 'utf8').toString('base64'), 'application/json', `kamino/${veilig}.json`);
...
async function _ensureSeedProjectVoorLead(token, rec) {
  ...
  try {
    ...
    if (!bestaat || !bestaat.id) {
      const nu = new Date().toISOString();
      const seed = {
        id, naam: rec.naam || 'Energie-Compas-lead',
...
app.post('/api/lead', async (req, res) => {
  try {
    ...
    const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
      ...
      <p>Met vriendelijke groeten,<br>Energie-Compas</p></div>`;
  ...
  try {
    ...
    if (optProfiel && /^\d{18}$/.test(eanEff) && SUPABASE_OK) {
      try {
        ...
        if (en && bezig.indexOf(en.status) >= 0) { eanQueued = true; }
          ...
          try { await _brevoMail(LEAD_MAIL_TO, `EK-snel mandaat — ${rec.naam || rec.mail} (EAN ${eanEff})`, lTxt, null, 'Energie-Compas'); } catch (e) {}
        }
    ...
    if (optContact && _validMail(rec.mail)) {
      try {
        ...
        const kTxt = `Beste${rec.naam ? ' ' + rec.naam : ''},\n\nUw Energie-Compas-rapport staat klaar:\n${link}\n\nZodra uw kwartierdata binnen is, werken we het automatisch bij.\n\nMet vriendelijke groeten,\nEnergie-Compas`;
        await _brevoMail(rec.mail, 'Uw Energie-Compas-rapport', kTxt, null, null, 'rapport');
      } catch (e) {}
    }
...
// v15.158.2: RAPPORT MAILEN (EK cut-the-crap) — knop in ek.html. Mailt de deeplink naar dit rapport (ek.html?case=<token>)
//   via de EK-mailbox (Graph, geen mailinglijst-banner), zonder de OTP-'verified'-gate (de EK-flow ankert op de case-token,
//   niet op e-mail-OTP). Throttled 5 min per lead. Klikken = expliciete klant-actie → we mailen naar het opgegeven adres.
app.post('/api/ek-rapport-mail', async (req, res) => {
  try {
    const b = req.body || {}; const token = String(b.token || '').trim();
    const rec = token ? _leadLezen(token) : null;
    const mail = _validMail(b.mail) ? String(b.mail).trim() : (rec && rec.mail);
    if (!_validMail(mail)) return res.status(400).json({ ok: false, error: 'Geen geldig e-mailadres.' });
    const nu = Date.now();
    if (rec && rec.ek_rapport_mail_ts && (nu - rec.ek_rapport_mail_ts) < 5 * 60 * 1000) return res.json({ ok: true, throttled: true, mail });
    const link = token ? `${WEB_BASE}/apps/ek.html?case=${encodeURIComponent(token)}` : `${WEB_BASE}/apps/ek.html`;
    const naam = (rec && rec.naam) ? ' ' + rec.naam : '';
    // v15.163: optionele PDF-bijlage (klantrapport, client-side gegenereerd). Accepteert data-uri of kale base64.
    let attachments = null, attached = false;
    try {
      let b64 = (b.pdf_base64 != null) ? String(b.pdf_base64) : '';
      const komma = b64.indexOf('base64,'); if (komma >= 0) b64 = b64.slice(komma + 7);
      b64 = b64.replace(/\s+/g, '');
      if (b64 && /^[A-Za-z0-9+/=]+$/.test(b64)) {
        const bytes = Buffer.byteLength(b64, 'base64');
        if (bytes > 0 && bytes <= 8 * 1024 * 1024) {
          const fn = String(b.filename || 'Energie-analyse.pdf').replace(/[^A-Za-z0-9._-]/g, '_').slice(0, 80) || 'Energie-analyse.pdf';
          attachments = [{ name: fn.toLowerCase().endsWith('.pdf') ? fn : fn + '.pdf', contentType: 'application/pdf', contentBytes: b64 }];
          attached = true;
        }
      }
    } catch (e) { attachments = null; attached = false; }
    const kTxt = attached
      ? `Beste${naam},\n\nIn bijlage vindt u uw Energie-Compas-rapport (PDF). U kunt het ook online bekijken:\n${link}\n\nZodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.\n\nMet vriendelijke groeten,\nEnergie-Compas`
      : `Beste${naam},\n\nUw Energie-Compas-rapport staat klaar:\n${link}\n\nZodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.\n\nMet vriendelijke groeten,\nEnergie-Compas`;
    const kHtml = attached
      ? `<p>Beste${esc2(naam)},</p><p>In bijlage vindt u uw Energie-Compas-rapport (PDF). U kunt het ook <a href="${esc2(link)}">online bekijken</a>.</p><p>Zodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.</p><p>Met vriendelijke groeten,<br>Energie-Compas</p>`
      : `<p>Beste${esc2(naam)},</p><p>Uw Energie-Compas-rapport staat klaar:<br><a href="${esc2(link)}">${esc2(link)}</a></p><p>Zodra uw kwartierdata (en eventuele offertes) binnen zijn, werken we het automatisch bij.</p><p>Met vriendelijke groeten,<br>Energie-Compas</p>`;
    let sent = false; try { const r = await _ekKlantMail(mail, 'Uw Energie-Compas-rapport', kTxt, kHtml, attachments); sent = !!(r && r.sent); } catch (e) {}
    if (rec) { rec.ek_rapport_mail_ts = nu; _leadOpslaan(token, rec); try { _leadEvent(rec, 'ek_rapport_mail', { attached }, token); } catch (e) {} }
    return res.json({ ok: true, mail, sent, attached, throttled: false });
  } catch (e) { console.error('[ek-rapport-mail]', e.message); res.status(500).json({ ok: false, error: e.message }); }
});
app.post('/api/lead-interesse', async (req, res) => {
  try {
    ...
    if (b.wil_contact) {
      const lTxt = `WARME LEAD — wil weten hoe (Energie-Compas${rec.partner ? ' ' + rec.partner : ''})\n\nNaam : ${rec.naam || '—'}\nMail : ${rec.mail}\nTel  : ${rec.tel || '—'}\n\nInteresses:\n${int.map(i => '  • ' + i).join('\n') || '  (geen)'}\n\nAfname-marge ${_eurTxt(rec.samenvatting.afname_marge_jaar)}/j · injectie ${_eurTxt(rec.samenvatting.injectie_marge_jaar)}/j`;
      await _brevoMail(LEAD_MAIL_TO, `WARME LEAD Energie-Compas — ${rec.naam || rec.mail}`, lTxt, null, 'Energie-Compas');
    }
    res.json({ ok: true });
...
app.post('/api/lead-herbereken', async (req, res) => {
  try {
    ...
    // niet naar een losse nota. Het rapport zit NIET in de mail — de klant komt terug in Energie-Compas, waar hij het
    ...
    const kTxt = `Beste${rec.naam ? ' ' + rec.naam : ''},\n\n${echt ? 'Uw Fluvius-kwartierdata is verwerkt. ' : ''}Open uw ${echt ? 'exacte ' : ''}rapport in Energie-Compas — daar staat uw volledige analyse (op uw eigen profiel) en zet u meteen de volgende stap:\n${appUrl}\n\nMet vriendelijke groeten,\nEnergie-Compas`;
...
// vooruitzet; de klant krijgt zijn rapport per mail als DEEPLINK naar Energie-Compas (niet als bijlage) — reden om
...
app.post('/api/lead-rapport-mail', async (req, res) => {
  try {
    ...
    const kop = 'Uw Energie-Compas-rapport';
    const kTxt = `Beste${rec.naam ? ' ' + rec.naam : ''},\n\nUw rapport staat klaar in Energie-Compas — met alles wat u tot nu toe toevoegde. Hoe meer databronnen u koppelt (uw profiel, uw wagenpark), hoe scherper het rapport:\n${appUrl}\n\nMet vriendelijke groeten,\nEnergie-Compas`;
    const kSent = await _brevoMail(rec.mail, kop, kTxt, null, null, 'rapport');
...
app.post('/api/lead-verify-send', async (req, res) => {
  try {
    ...
    _leadOpslaan(b.token, rec);
    const txt = `Uw verificatiecode voor Energie-Compas is: ${code}\n\nGeef deze code in op de pagina om uw analyse te bekijken. De code is 15 minuten geldig.`;
    const html = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864"><p>Uw verificatiecode voor <b>Energie-Compas</b>:</p><p style="font-size:30px;font-weight:800;letter-spacing:4px;color:#05B050">${code}</p><p style="color:#5A6577;font-size:13px">Geef deze in op de pagina om uw analyse te bekijken. 15 minuten geldig.</p></div>`;
    const sent = await _brevoMail(rec.mail, 'Uw Energie-Compas-verificatiecode', txt, html);
...
// met een verder-link naar Energie-Compas. Eenmalige klantmail (guard klant_verwittigd_analoog). Mail-review-gated.
...
app.post('/api/lead-bevraging', async (req, res) => {
  try {
    ...
    // v15.156.8: leg de tweede-rapport-intentie vast — het definitieve rapport draait zodra profieldata (kwartierdata)
    //   + offertes binnen zijn; kostbron = 'offerte' (échte leveranciersprijzen) indien offertes aangevraagd, anders
    //   'eerste_simulatie'. + markeer het gekoppelde project zodat het als actie zichtbaar is (offertes → partners).
    rec.rapport2 = { gepland: true, wacht_op: ['profieldata', 'offertes'], kostbron: 'offerte', ts: Date.now() };
    if (rec.kamino_project_id) {
      try {
        const { veilig, rec: pr } = await _kaminoRecLaden(rec.kamino_project_id);
        if (pr) { pr.offertes_aangevraagd = { ts: new Date().toISOString(), samenstelling: sm }; pr.bijgewerkt = new Date().toISOString();
    ...
    const lTxt = `ANONIEME OFFERTE-BEVRAGING aangevraagd (Energie-Compas${rec.partner ? ' ' + rec.partner : ''})\n\nNaam : ${rec.naam || '—'}\nMail : ${rec.mail}\nTel  : ${rec.tel || '—'}${smLijn}\n\n→ Leg het dossier GEANONIMISEERD (mini-bestek) voor aan de betrouwbare partners; bezorg de klant de offertes ter vergelijking. KU Leuven-verificatie enkel op de partnerofferte.\n\nAfname-marge ${_eurTxt(s.afname_marge_jaar || 0)}/j${rec.kamino_project_id ? ('\nKamino-project: ' + rec.kamino_project_id) : ''}`;
    await _brevoMail(LEAD_MAIL_TO, `BEVRAGING Energie-Compas — ${rec.naam || rec.mail}`, lTxt, null, 'Energie-Compas');
    res.json({ ok: true });
  } catch (e) { console.error('[lead-bevraging]', e.message); res.status(500).json({ ok: false, error: e.message }); }
});
// v15.157 (fase 2): MANAGER markeert dat de offertes voor een lead BINNEN zijn → probeert meteen de "simulatie klaar"-
// mail te triggeren (als de kwartierdata er ook is). Voor de flow waarin de klant offertes vroeg en Johan ze manueel
// bij de partners ophaalde. { token }.
app.post('/api/lead-offertes-ontvangen', async (req, res) => {
  try {
    const u = await _managerGuard(req, res); if (!u) return;
    const b = req.body || {}; const token = String(b.token || '').trim();
    const rec = _leadLezen(token); if (!rec) return res.status(404).json({ ok: false, error: 'Lead niet gevonden.' });
    rec.offertes_ontvangen = true; rec.offertes_ontvangen_ts = Date.now();
    try { _leadEvent(rec, 'offertes_ontvangen', {}, token); } catch (e) {}
    _leadOpslaan(token, rec);
    const rapport2 = await _rapport2Trigger(token, 'offertes-ontvangen');
    res.json({ ok: true, rapport2 });
  } catch (e) { console.error('[lead-offertes-ontvangen]', e.message); res.status(500).json({ ok: false, error: e.message }); }
});
...
app.post('/api/lead-afsluiten', async (req, res) => {
  try {
    ...
    const lTxt = `AFSLUITING Energie-Compas${rec.partner ? ' (' + rec.partner + ')' : ''}\n\nNaam : ${rec.naam || '—'}\nMail : ${rec.mail}\nTel  : ${rec.tel || '—'}\nOfferte: ${partner ? 'PARTNER (incl. KU Leuven)' : 'geen partner (verificatie zonder KU Leuven)'}\n\n${acties.join('\n')}${rec.kamino_project_id ? ('\n\nKamino-project: ' + rec.kamino_project_id) : ''}`;
    await _brevoMail(LEAD_MAIL_TO, `AFSLUITING Energie-Compas — ${rec.naam || rec.mail}`, lTxt, null, 'Energie-Compas');
    _leadOpslaan(b.token, rec);
    res.json({ ok: true });
...
// kerncijfers af tegen de Energie-Compas-studie van deze lead (second opinion). Geen financieel advies:
...
function _leadVanEanKlant(ean, klantNaam, klantBtw) {
  ...
  for (const [token, rec] of _LEADS) {
    ...
    if (rec.archief) continue;   // v15.157.3: gearchiveerd dossier = met rust laten → nieuwe factuur start een NIEUW project (Johan, optie B)
...
// v15.157 (fase 2): "SIMULATIE KLAAR"-TRIGGER. Zodra het gemeten profiel (kwartierdata) op het project staat en —
// indien de klant offertes vroeg — die offertes ontvangen zijn, mailen we de klant dat het definitieve rapport op
// zijn eigen verbruiksdata klaar is, met de link naar de KPI-pagina (ek.html?case=<token>, die live op het echte
// profiel rekent via /api/kamino/onderhandel → _pasOpgeladenAfnameToe). Kostbron = offerteprijzen indien ontvangen,
// anders de kostprijs uit de eerste simulatie (factuur). Eenmalig per lead (rapport2.klaar_mail_ts). Niet-blokkerend.
async function _rapport2Trigger(token, reden) {
  try {
    if (!token) return { ok: false, reden: 'geen token' };
    const rec = _leadLezen(token); if (!rec) return { ok: false, reden: 'lead niet gevonden' };
    if (rec.rapport2 && rec.rapport2.klaar_mail_ts) return { ok: false, reden: 'al gemaild' };   // eenmalig
    if (rec.contact_consent === false) return { ok: false, reden: 'geen contact-consent' };
    if (!_validMail(rec.mail)) return { ok: false, reden: 'geen geldig adres' };
    // Profiel (kwartierdata) aanwezig op het gekoppelde project?
    let profielOk = false;
    if (rec.kamino_project_id) { try { const kp = await _kaminoRecLaden(rec.kamino_project_id); profielOk = !!(kp && kp.rec && kp.rec.profielen && kp.rec.profielen.afname && kp.rec.profielen.afname.bestand); } catch (e) {} }
    if (!profielOk) return { ok: false, reden: 'nog geen kwartierdata-profiel op project' };
    // Wachten op offertes? Enkel als de klant ze vroeg én ze nog niet ontvangen zijn.
    const offertesGevraagd = !!(rec.bevraging && rec.bevraging.aangevraagd);
    if (offertesGevraagd && !rec.offertes_ontvangen) return { ok: false, reden: 'wacht op offertes' };
    const kostbron = (offertesGevraagd && rec.offertes_ontvangen) ? 'offerte' : 'eerste_simulatie';
    const link = _ekLink(token);
    const naam = rec.naam ? (' ' + rec.naam) : '';
    const bronLijn = kostbron === 'offerte'
      ? 'op basis van uw eigen kwartierdata én de ontvangen leveranciersprijzen'
      : 'op basis van uw eigen kwartierdata van Fluvius';
    const kTxt = `Beste${naam},\n\nUw definitieve energie-analyse is klaar — berekend ${bronLijn}. Dit is uw rapport op maat, exacter dan de eerste raming.\n\nBekijk uw KPI-pagina:\n${link}\n\nMet vriendelijke groeten,\nEnergie-Compas`;
    const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
      <p>Beste${naam ? ' ' + esc2(rec.naam) : ''},</p>
      <p>Uw <b>definitieve energie-analyse</b> is klaar — berekend ${esc2(bronLijn)}. Dit is uw rapport op maat, exacter dan de eerste raming.</p>
      <p><a href="${link}" style="display:inline-block;background:#05B050;color:#fff;text-decoration:none;font-weight:700;padding:11px 18px;border-radius:8px">Bekijk uw KPI-pagina →</a></p>
      <p>Met vriendelijke groeten,<br>Energie-Compas</p></div>`;
    const kSent = await _ekKlantMail(rec.mail, 'Uw definitieve analyse met uw eigen verbruiksdata is klaar', kTxt, kHtml);
    rec.rapport2 = Object.assign({}, rec.rapport2 || {}, { gepland: true, klaar: true, kostbron, klaar_mail_ts: Date.now(), klaar_mail_kanaal: kSent.kanaal || null, reden: reden || null });
    try { _leadOpslaan(token, rec); } catch (e) {}
    try { _leadEvent(rec, 'rapport2_klaar_gemaild', { extra: { kostbron, kanaal: kSent.kanaal } }, token); } catch (e) {}
    // Back-office-notify (intern, via Brevo).
    try { await _brevoMail(LEAD_MAIL_TO, `Rapport 2 gemaild — ${rec.naam || rec.mail}`, `Definitief rapport (kwartierdata) gemaild naar ${rec.mail} (kanaal ${kSent.kanaal}).\nKostbron: ${kostbron}.\nProject: ${rec.kamino_project_id || '—'}\nLink: ${link}`, null, 'Energie-Compas'); } catch (e) {}
    console.log(`[rapport2] "simulatie klaar" gemaild naar ${rec.mail} (kostbron ${kostbron}, kanaal ${kSent.kanaal})`);
    return { ok: true, kostbron, kanaal: kSent.kanaal };
  } catch (e) { console.warn('[rapport2] trigger faalde (niet-blokkerend):', e.message); return { ok: false, reden: e.message }; }
}
...
async function _graphAuthAlert(err) {
  ...
  try { await _brevoMail(LEAD_MAIL_TO, 'Graph-intake faalt — actie vereist', txt, null, 'Energie-Compas'); } catch (e) {}
...
async function _graphInboundSweep() {
  ...
  for (const m of relevant) {
    ...
    try {
      ...
      if (!pdfs.length) { res.status = 'geen_pdf'; st.verwerkt[m.id] = { status: 'klaar', substatus: 'geen_pdf', pogingen: pogingen + 1, ontvangen: m.ontvangen, gemeld: true, from: m.from, subject: m.subject }; await graphInbound.markRead(m.id); uit.push(res); console.log(`[graph-inbound] → geen PDF, afgehandeld`); continue; }
      ...
      if (!apiKey) { res.status = 'geen_apikey'; st.verwerkt[m.id] = { status: 'fout', substatus: 'geen_apikey', pogingen: pogingen + 1, ontvangen: m.ontvangen, gemeld: false, from: m.from, subject: m.subject }; uit.push(res); console.error(`[graph-inbound] → GEEN ANTHROPIC_API_KEY — kan factuur niet lezen (retry volgt)`); continue; }
      ...
      if (hit) {
        ...
        // v15.156.6: mandaat-wachtrij niet automatisch — pas na bevestiging op EK (consent-first).
        ...
        if (rec.abonnement_opt_out) {
          ...
          const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
            ...
            <p>Met vriendelijke groeten,<br>Energie-Compas</p></div>`;
          const kSent = await _ekKlantMail(mailNaar, 'Uw energie-analyse is bijgewerkt', kTxt, kHtml);   // v15.157: via EK-mailbox (Graph) → geen mailinglijst-banner (factuurmail bewijst adrescontrole; Johan: "rapporten mogen doorgaan")
        ...
        // v15.156.6: GEEN automatische mandaat-wachtrij bij factuurontvangst — de EAN komt pas "aan te vragen" NADAT de
        //   klant op EK bevestigt (POST /api/ek-intake, met kwartierdata-opt-in). Consent-first (Johan).
        const link = _ekLink(token);   // v15.156: cut-the-crap → ek.html
        ...
        if (autoreport) {
          ...
          const kHtml = `<div style="font:15px/1.55 Helvetica,Arial,sans-serif;color:#1F3864;max-width:560px">
            ...
            <p style="background:#F4F7FB;border:1px solid #D6E0EC;border-radius:9px;padding:10px 12px;font-size:13px;color:#1F3864"><b>💡 Gratis up-to-date:</b> stelt u in de mailbox waar u uw elektronische factuur ontvangt een automatische doorstuur in naar <b>dossier@energie-compas.eu</b>, dan werken we uw analyse bij elke nieuwe factuur automatisch bij.</p>
            <p>Met vriendelijke groeten,<br>Energie-Compas</p></div>`;
          const kSent = await _ekKlantMail(m.from, 'We lazen uw factuur — vul even aan voor uw analyse', kTxt, kHtml);   // v15.157: via EK-mailbox (Graph) → geen mailinglijst-banner
          ...
          await _brevoMail(LEAD_MAIL_TO, `Abonnement — NIEUWE klant via factuurmail (${bc.klantNaam || m.from})`, bTxt, null, 'Energie-Compas');
        } else {
          ...
          await _brevoMail(LEAD_MAIL_TO, `Abonnement — nieuwe klant via factuurmail (DRY-RUN) (${bc.klantNaam || m.from})`, bTxt, null, 'Energie-Compas');
        }
      } else {
        ...
        await _brevoMail(LEAD_MAIL_TO, `Abonnement — inkomende factuur (manueel) (${bc.klantNaam || m.from})`, txt, null, 'Energie-Compas');
      }
      await graphInbound.markRead(m.id);
      st.verwerkt[m.id] = { status: 'klaar', substatus: res.status, pogingen: pogingen + 1, ontvangen: m.ontvangen, gemeld: true, token: res.token || null, from: m.from, subject: m.subject, ean: res.ean || null, klant: res.klant || null };
      ...
      if (nP >= MAX_POGINGEN) {
        ...
        const fTxt = `⚠️ FACTUURMAIL-ABONNEMENT — factuur KON NIET verwerkt worden\n\n`
          ...
          + `→ Handmatig opvolgen (bv. factuur opnieuw/anders opladen in Energie-Compas). De app probeert deze mail niet meer automatisch.`;
        try { await _brevoMail(LEAD_MAIL_TO, `Abonnement — factuur MISLUKT (${m.from})`, fTxt, null, 'Energie-Compas'); } catch (_) {}
        st.verwerkt[m.id] = { status: 'fout_opgegeven', pogingen: nP, ontvangen: m.ontvangen, gemeld: true, fout: e.message, from: m.from, subject: m.subject };
        ...
        st.verwerkt[m.id] = { status: 'fout', pogingen: nP, ontvangen: m.ontvangen, gemeld: false, fout: e.message, from: m.from, subject: m.subject };
...
// v15.156.7: DIAGNOSE-LOG per verwerkte factuurmail — wat is er met elke mail gebeurd (nieuwe lead / abonnement /
//   geen EAN / fout)? Leest de durabele graph-state (abonnement/graph_state.json → st.verwerkt). Manager-only.
app.get('/api/graph/inbound-log', async (req, res) => {
  try {
    if (!(await _isManagerReq(req))) return res.status(401).json({ ok: false, error: 'Manager-login vereist' });
    let st = null; try { st = JSON.parse(await _factuurDownload('abonnement/graph_state.json', true)); } catch (e) {}
    const v = (st && st.verwerkt) || {};
    const rijen = Object.keys(v).map(id => { const e = v[id] || {};
      return { id, from: e.from || null, subject: e.subject || null, status: e.substatus || e.status || null,
        ean: e.ean || null, klant: e.klant || null, token: e.token || null, pogingen: e.pogingen || 0,
        fout: e.fout || null, ontvangen: e.ontvangen || null }; })
      .sort((a, b) => String(b.ontvangen || '').localeCompare(String(a.ontvangen || ''))).slice(0, 60);
    res.json({ ok: true, aantal: rijen.length, mails: rijen, _meta: { server_version: SERVER_VERSIE } });
  } catch (e) { console.error('[inbound-log]', e.message); res.status(500).json({ ok: false, error: e.message }); }
});
...
app.get('/api/inbound-case/:token', async (req, res) => {
  try {
    ...
    const gedeeld = { ok: true, verified: !!rec.verified, mail_otp_pending: !!rec.mail_otp_pending, mail_masked: _maskMail(rec.mail),
      ...
      ev: (rec.ev != null ? +rec.ev : null), profiel: rec.profielNaam || null,   // v15.157.1: onthoud de vorige klant-input (aantal wagens + gekozen profiel) → voorvullen bij terugkeer
...
app.get('/api/abonnement/unsub', (req, res) => {
  const pagina = (titel, body) => `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><div style="font:16px/1.6 Helvetica,Arial,sans-serif;color:#1F3864;max-width:520px;margin:44px auto;padding:0 20px"><h2 style="color:#1F3864;margin-bottom:8px">${titel}</h2>${body}<p style="color:#8A93A3;font-size:13px;margin-top:24px">Energie-Compas — Fluctus.net CVSO</p></div>`;
  try {
    ...
    res.send(pagina('U bent uitgeschreven', '<p>U ontvangt geen automatische maandelijkse analyses meer. Uw dossier blijft bewaard — u kunt zich later opnieuw inschrijven via de Energie-Compas-app.</p>'));
...
// MANAGER-ONLY: overzicht van de Energie-Compas-leads voor de supermanager/accountmanagers.
...
async function _scanOcm(lat, lon) {
  ...
  const r = await fetch(url, { headers: { 'User-Agent': 'FluctusEnergie-Compas/1.0' } }); if (!r.ok) return null;
