...
async function _ekBedrijfRapport(ctx, pv, bkw) {
  const [rNul, rGeen, rStuur, rOnb] = await Promise.all([
    _ekRapportRun(ctx, 0, 0, 'geen'),        // nulscenario: geen PV/batt, dom laden
  ...
  const fNul = _factuurBits(rNul), fOpt = _factuurBits(rStuur);
  const besp = fNul.saldo - fOpt.saldo;
  const factuur = { nul: fNul, opt: fOpt, besparing_jaar: besp, besparing_pct: fNul.saldo > 0 ? Math.round(besp / fNul.saldo * 100) : 0 };
