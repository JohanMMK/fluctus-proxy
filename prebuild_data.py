...
Periode: dynamisch bepaald op basis van de laatste volledige dag in de caches.
- Tot = meest beperkende laatste volledige dag (min van spot/imb/solar)
- Van = Tot - 365 dagen (exact 1 jaar terug)
- Solar_norm: volledig kalenderjaar 2025 (jan-dec), voor quarter_index lookup
"""
import json, sys
    ...
    for zoekpad in [
        ...
        if zoekpad.exists():
            with open(zoekpad) as f:
                data = json.load(f)
            sys.stderr.write(f"OK: {zoekpad} ({len(data)} entries)\n")
...
def laatste_volledige_dag(timestamps_ms):
    """Geeft de laatste VOLLEDIGE dag terug (als date) op basis van de timestamps."""
    if not timestamps_ms:
        return None
    last_ts = max(timestamps_ms)
    last_dt = datetime.fromtimestamp(last_ts / 1000, tz=timezone.utc)
    # Als de laatste timestamp niet op het einde van de dag staat (23:45):
    # neem de dag ervoor als laatste volledige dag
...
# Neem de meest beperkende (kleinste) datum
beschikbare = [d for d in [laatste_spot, laatste_imb, laatste_solar] if d is not None]
if not beschikbare:
    sys.stderr.write("WARN: geen enkele cache gevonden — gebruik fallback periode\n")
    # Fallback: afgelopen jaar t/m gisteren
    from datetime import date
    laatste_dag = date.today() - timedelta(days=1)
else:
    laatste_dag = min(beschikbare)

# Periode: van = laatste_dag - 365 dagen, tot = laatste_dag + 1 dag (exclusief)
...
sys.stderr.write(f"Simulatieperiode: {VAN_MARKT.date()} → {(TOT_MARKT - timedelta(seconds=1)).date()} (inclusief)\n")

# Spot + imbalance: exacte periode van→tot
spot_q, imb_q = [], []
cur = VAN_MARKT
while cur < TOT_MARKT:
...
# Solar norm: volledig kalenderjaar 2025 (35040 kwartieren)
# De simulator gebruikt quarter_index_in_year_2025() om de juiste waarde te vinden
...
# Profiel laden (slager als default)
profiel = []
for naam in ['data/profielen/Slager.json', 'data/profielen/slager.json', 'data/slager.json', 'slager.json']:
    p = BASE / naam
    if p.exists():
        with open(p) as f:
            profiel = json.load(f)
        ...
        break
if not profiel:
    sys.stderr.write("WARN: geen profiel gevonden\n")

out = {
    'spot_q':          spot_q,
    'imb_q':           imb_q,
    'solar_norm':      solar_norm,   # 35040 waarden voor 2025-kalender
    'profiel':         profiel,
    'van':             VAN_MARKT.strftime('%Y-%m-%d'),
    'tot':             (TOT_MARKT - timedelta(days=1)).strftime('%Y-%m-%d'),  # inclusieve einddatum
...
sys.stderr.write(f"OK: {out_path} — periode {out['van']} t/m {out['tot']}, {out['n_kwartieren']} kwartieren\n")
print(out_path)
